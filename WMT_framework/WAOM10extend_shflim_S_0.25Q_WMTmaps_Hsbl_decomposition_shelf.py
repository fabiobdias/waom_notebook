...
#    salt_net_tmp = np.nanmean(Fs_rate_delta_net_vint_shelf_mm[:,irho,:], axis=0)*rho0/(dx*dy)
    ...
    for mm in np.arange(0,12):
        m_sfc_tmp[mm,:] = Fm_rate_delta_sfc_shelf_mm[mm,irho,:]*rho0/(dx*dy)
...
name_fig="waom10extend_shflim_S_0.25Q_WMTmaps_salt_budget_annual_yr20_shelf.png"
...
#    plt.clim(-2e-6,2e-6)
    ...
    name_fig="waom10extend_shflim_S_0.25Q_WMTmaps_salt_budget_annual_yr20_shelf_mm=" + str(mm) + ".png"
    plt.savefig(fig_path + name_fig, dpi=300)
    plt.close()
