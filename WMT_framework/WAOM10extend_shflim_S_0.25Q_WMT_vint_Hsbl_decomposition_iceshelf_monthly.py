...
Ff_rate_delta_adv_vint_iceshelf_mm = wmt(fwf_adv_mld_vint*mask_outice, dx, dy,'budget')
Ff_rate_delta_diff_vint_iceshelf_mm = wmt(R_f_vint*mask_outice, dx, dy,'budget')
Ff_rate_delta_net_vint_iceshelf_mm = wmt(fwf_net_mld_vint*mask_outice, dx, dy,'budget')
Ff_rate_delta_sfc_iceshelf_mm = wmt(fwf_sfc*mask_outice, dx, dy,'sfc_frc')
...
for irho in np.arange(0,len(rho_grid)):   
    for mm in np.arange(0,12):
        ...
        Ff_rate_sfc_iceshelf_mm_int[irho,mm] = np.nansum(np.nansum(Ff_rate_delta_sfc_iceshelf_mm[mm,irho,:], axis=1), axis=0)
        Fh_rate_sfc_iceshelf_mm_int[irho,mm] = np.nansum(np.nansum(Fh_rate_delta_sfc_iceshelf_mm[mm,irho,:], axis=1), axis=0)
        Fm_rate_sfc_iceshelf_mm_int[irho,mm] = np.nansum(np.nansum(Fm_rate_delta_sfc_iceshelf_mm[mm,irho,:], axis=1), axis=0)
        
        Fs_rate_adv_vint_iceshelf_mm_int[irho,mm] = np.nansum(np.nansum(Fs_rate_delta_adv_vint_iceshelf_mm[mm,irho,:], axis=1), axis=0)
        Ff_rate_adv_vint_iceshelf_mm_int[irho,mm] = np.nansum(np.nansum(Ff_rate_delta_adv_vint_iceshelf_mm[mm,irho,:], axis=1), axis=0)
        ...
        Ff_rate_diff_vint_iceshelf_mm_int[irho,mm] = np.nansum(np.nansum(Ff_rate_delta_diff_vint_iceshelf_mm[mm,irho,:], axis=1), axis=0)
        ...
        Ff_rate_net_vint_iceshelf_mm_int[irho,mm] = np.nansum(np.nansum(Ff_rate_delta_net_vint_iceshelf_mm[mm,irho,:], axis=1), axis=0)
...
Fs_sig_sfc_iceshelf_mm = -Fs_rate_sfc_iceshelf_mm_int*rho0/1e6
Fh_sig_sfc_iceshelf_mm = -Fh_rate_sfc_iceshelf_mm_int*rho0/1e6 
Fm_sig_sfc_iceshelf_mm = -Fm_rate_sfc_iceshelf_mm_int*rho0/1e6
F_sig_sfc_iceshelf_mm = -Fs_sig_sfc_iceshelf_mm + Fh_sig_sfc_iceshelf_mm + Fm_sig_sfc_iceshelf_mm
...
# ADV (vint)
Fs_sig_adv_vint_iceshelf_mm = -Fs_rate_adv_vint_iceshelf_mm_int*rho0/1e6 
Fh_sig_adv_vint_iceshelf_mm = -Fh_rate_adv_vint_iceshelf_mm_int*rho0/1e6 
F_sig_adv_vint_iceshelf_mm = -Fs_sig_adv_vint_iceshelf_mm + Fh_sig_adv_vint_iceshelf_mm

# DIFF
Fs_sig_diff_vint_iceshelf_mm = -Fs_rate_diff_vint_iceshelf_mm_int*rho0/1e6 
Fh_sig_diff_vint_iceshelf_mm = -Fh_rate_diff_vint_iceshelf_mm_int*rho0/1e6 
F_sig_diff_vint_iceshelf_mm = -Fs_sig_diff_vint_iceshelf_mm + Fh_sig_diff_vint_iceshelf_mm

# NET
Fs_sig_net_vint_iceshelf_mm = -Fs_rate_net_vint_iceshelf_mm_int*rho0/1e6 
Fh_sig_net_vint_iceshelf_mm = -Fh_rate_net_vint_iceshelf_mm_int*rho0/1e6 
...
for mm in np.arange(0,12):
    ...
    cs=plt.plot(rho_grid,-Fs_sig_net_vint_iceshelf_mm[:,mm],'b',label='WMT due to salt tend')
    cs=plt.plot(rho_grid,-Fs_sig_diff_vint_iceshelf_mm[:,mm],'--b',label='WMT due to salt diff')
    cs=plt.plot(rho_grid,-Fs_sig_adv_vint_iceshelf_mm[:,mm],':b',label='WMT due to salt adv')
    cs=plt.plot(rho_grid,-Fs_sig_sfc_iceshelf_mm[:,mm] + Fm_sig_sfc_iceshelf_mm[:,mm],'--c',label='WMT due to salt sfc')
    if mm == 0:
        plt.legend()
...
name_fig="waom10extend_shflim_S_0.25Q_WMT_salt_vint_monthly_iceshelf.png"
...
for mm in np.arange(0,12):
    ax1 = fig.add_subplot(4,3,mm+1)
    plt.title(mm_names[mm])
    cs=plt.plot(rho_grid,Fh_sig_net_vint_iceshelf_mm[:,mm],'r',label='WMT due to heat tend')
    cs=plt.plot(rho_grid,Fh_sig_diff_vint_iceshelf_mm[:,mm],'--r',label='WMT due to heat diff')
    cs=plt.plot(rho_grid,Fh_sig_adv_vint_iceshelf_mm[:,mm],':r',label='WMT due to heat adv')
    cs=plt.plot(rho_grid,Fh_sig_sfc_iceshelf_mm[:,mm],'--m',label='WMT due to heat sfc')
    if mm == 0:
        plt.legend()
    if mm == 0 or mm == 3 or mm == 6 or mm == 9:
        plt.ylabel('Transformation rate (Sv)')
...
name_fig="waom10extend_shflim_S_0.25Q_WMT_heat_vint_monthly_iceshelf.png"
plt.savefig(fig_path + name_fig, dpi=300)
...
for mm in np.arange(0,12):
    ax1 = fig.add_subplot(4,3,mm+1)
    plt.title(mm_names[mm])
    cs=plt.plot(rho_grid,F_sig_net_vint_iceshelf_mm[:,mm],'k',label='WMT due to total tend')
    cs=plt.plot(rho_grid,F_sig_diff_vint_iceshelf_mm[:,mm],'--k',label='WMT due to total diff')
    cs=plt.plot(rho_grid,F_sig_adv_vint_iceshelf_mm[:,mm],':k',label='WMT due to total adv')
    cs=plt.plot(rho_grid,F_sig_sfc_iceshelf_mm[:,mm],'--g',label='WMT due to total sfc')
    if mm == 0:
        plt.legend()
    if mm == 0 or mm == 3 or mm == 6 or mm == 9:
        plt.ylabel('Transformation rate (Sv)')
...
name_fig="waom10extend_shflim_S_0.25Q_WMT_total_vint_monthly_iceshelf.png"
plt.savefig(fig_path + name_fig, dpi=300)
...
#    cy=plt.pcolormesh(lon_rho,lat_rho,np.nanmean(Fs_rate_delta_net_vint_iceshelf_mm[:,irho,:], axis=0)*rho0/(dx*dy), transform=ccrs.PlateCarree(), cmap=plt.cm.coolwarm)
#    plt.colorbar(cy)
...
#    cy=plt.pcolormesh(lon_rho,lat_rho,np.nanmean(Fh_rate_delta_net_vint_iceshelf_mm[:,irho,:], axis=0)*rho0/(dx*dy), transform=ccrs.PlateCarree(), cmap=plt.cm.coolwarm)
#    plt.colorbar(cy)
...
#    cy=plt.pcolormesh(lon_rho,lat_rho,np.nanmean(Fh_rate_delta_net_vint_iceshelf_mm[:,irho,:], axis=0)*rho0/(dx*dy)+np.nanmean(Fs_rate_delta_net_vint_iceshelf_mm[:,irho,:], axis=0)*rho0/(dx*dy), transform=ccrs.PlateCarree(), cmap=plt.cm.coolwarm)
#    plt.colorbar(cy)
...
#    plt.clim(-5e-6,5e-6)
#                                                
#    name_fig="waom10extend_shflim_S_0.25Q_WMTmaps_annual_yr20_l" + str(irho)  + "_iceshelf.png"
#    plt.savefig(fig_path + name_fig, dpi=300)
#    plt.close()
#
#proj = ccrs.SouthPolarStereo()
...
#plt.clim(-5e-6,5e-6)
#                                                
#name_fig="waom10extend_shflim_S_0.25Q_WMTmaps_annual_yr20_iceshelf.png"
#plt.savefig(fig_path + name_fig, dpi=300)
#plt.close()
