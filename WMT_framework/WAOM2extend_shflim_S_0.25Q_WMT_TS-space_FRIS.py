...
dx = xr.open_dataset('/scratch/project_2000339/boeiradi/waom2extend_shflim_S_0.25Q/output_yr5/MLD_vint_vars_for_WMT_m.s-1.nc')
...
# West Weddell = (xi_rho=876, eta_rho=slice(320,389))
# East Weddell = (xi_rho=1000, eta_rho=slice(330,394))
# SO, new west one:
# West Weddell = (xi_rho=850, eta_rho=slice(320,389))
# East Weddell = (xi_rho=1000, eta_rho=slice(330,394))

salt_net_WWed = salt_net_mld_vint.isel(xi_rho=slice(825,875), eta_rho=slice(1600,1946))
salt_adv_WWed = salt_adv_mld_vint.isel(xi_rho=slice(825,875), eta_rho=slice(1600,1946))
salt_diff_WWed = R_s_vint.isel(xi_rho=slice(825,875), eta_rho=slice(1600,1946))
salt_sfc_WWed = salt_sfc[:,1600:1946,825:875]
salt_mld_WWed = salt_mld[:,1600:1946,825:875]
temp_mld_WWed = temp_mld[:,1600:1946,825:875]
dx_WWed = dx[1600:1946,825:875]
dy_WWed = dy[1600:1946,825:875]

salt_net_EWed = salt_net_mld_vint.isel(xi_rho=slice(975,1025), eta_rho=slice(1670,1970))
salt_adv_EWed = salt_adv_mld_vint.isel(xi_rho=slice(975,1025), eta_rho=slice(1670,1970))
salt_diff_EWed = R_s_vint.isel(xi_rho=slice(975,1025), eta_rho=slice(1670,1970))
salt_sfc_EWed = salt_sfc[:,1670:1970,975:1025]
salt_mld_EWed = salt_mld[:,1670:1970,975:1025]
temp_mld_EWed = temp_mld[:,1670:1970,975:1025]
dx_EWed = dx[1670:1970,975:1025]
dy_EWed = dy[1670:1970,975:1025]
...
def wmt_TS(var_int, temp, salt, dx, dy,var_type):
    ...
    F_TS_rate_delta_var_vint_mm = np.empty((12,len_tbin,len_sbin,len(var_int[0,:]),50))
#     F_TS_rate_delta_var_vint_mm = np.empty((1,len_tbin,len_sbin,len(var_int[0,:])))
    ...
    for mm in np.arange(0,12):
        ...
        F_rate_delta = np.empty((len_tbin,len_sbin,len(var_int[0,:]),50))
...
#plt.colorbar()
plt.title('F) WAOM2 - Central FRIS')
ax = plt.gca()
...
name_fig="waom2extend_shflim_S_0.25Q_WMT_TS_RFISsections.png"
...
def custom_plotfunc(salt, fig, tt, *args, **kwargs):
    ...
    plt.title('B) WAOM2 - Central FRIS \n month = ' + str (tt))
    plt.xlabel('Salinity'),plt.ylabel('Temperature ($^{\circ}$C)')
    CS = plt.contour(Si,Ti,dens.transpose(), levels=np.arange(26.,28.2,.05),linestyles='dotted', colors='gray',linewidths=.5)
    plt.clabel(CS,fontsize=8)
    plt.xlim([34,34.7])
...
mov_custom.save('/users/boeiradi/COLD_project/postprocessing/figs/WMT/movie/WAOM2extend_WMT_TS-space_FRIS.mp4', overwrite_existing=True, progress=True, framerate=1)
