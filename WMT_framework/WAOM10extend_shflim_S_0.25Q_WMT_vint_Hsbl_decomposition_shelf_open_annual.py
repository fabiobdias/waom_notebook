...
Ff_rate_delta_adv_vint_shelf_mm = wmt(fwf_adv_mld_vint*mask_shelf, dx, dy,'budget')
Ff_rate_delta_diff_vint_shelf_mm = wmt(R_f_vint*mask_shelf, dx, dy,'budget')
Ff_rate_delta_net_vint_shelf_mm = wmt(fwf_net_mld_vint*mask_shelf, dx, dy,'budget')
Ff_rate_delta_sfc_shelf_mm = wmt(fwf_sfc*mask_shelf, dx, dy,'sfc_frc')
...
# Open-ocean only: excluding shelf

Fs_rate_delta_adv_vint_open_mm = wmt(salt_adv_mld_vint*mask_open, dx, dy,'budget')
Fs_rate_delta_diff_vint_open_mm = wmt(R_s_vint*mask_open, dx, dy,'budget')
Fs_rate_delta_net_vint_open_mm = wmt(salt_net_mld_vint*mask_open, dx, dy,'budget')
Fs_rate_delta_sfc_open_mm = wmt(salt_sfc*mask_open, dx, dy,'sfc_frc')
    
Ff_rate_delta_adv_vint_open_mm = wmt(fwf_adv_mld_vint*mask_open, dx, dy,'budget')
Ff_rate_delta_diff_vint_open_mm = wmt(R_f_vint*mask_open, dx, dy,'budget')
Ff_rate_delta_net_vint_open_mm = wmt(fwf_net_mld_vint*mask_open, dx, dy,'budget')
Ff_rate_delta_sfc_open_mm = wmt(fwf_sfc*mask_open, dx, dy,'sfc_frc')

Fh_rate_delta_adv_vint_open_mm = wmt(temp_adv_mld_vint*mask_open, dx, dy,'budget')
Fh_rate_delta_diff_vint_open_mm = wmt(R_t_vint*mask_open, dx, dy,'budget')
Fh_rate_delta_net_vint_open_mm = wmt(temp_net_mld_vint*mask_open, dx, dy,'budget')
Fh_rate_delta_sfc_open_mm = wmt(temp_sfc*mask_open, dx, dy,'sfc_frc')

Fm_rate_delta_sfc_shelf_mm = wmt(beta_mld*m*mask_shelf, dx, dy,'sfc_frc')

Fm_rate_delta_sfc_open_mm = wmt(beta_mld*m*mask_open, dx, dy,'sfc_frc')
...
# integrated over x, y directions: only open ocean
Fs_rate_sfc_open_mm_int = np.empty((len(rho_grid),12))
Ff_rate_sfc_open_mm_int = np.empty((len(rho_grid),12))
Fm_rate_sfc_open_mm_int = np.empty((len(rho_grid),12))
Fh_rate_sfc_open_mm_int = np.empty((len(rho_grid),12))

Fs_rate_adv_vint_open_mm_int = np.empty((len(rho_grid),12))
Ff_rate_adv_vint_open_mm_int = np.empty((len(rho_grid),12))
Fh_rate_adv_vint_open_mm_int = np.empty((len(rho_grid),12))
Fs_rate_diff_vint_open_mm_int = np.empty((len(rho_grid),12))
Ff_rate_diff_vint_open_mm_int = np.empty((len(rho_grid),12))
Fh_rate_diff_vint_open_mm_int = np.empty((len(rho_grid),12))
Fs_rate_net_vint_open_mm_int = np.empty((len(rho_grid),12))
Ff_rate_net_vint_open_mm_int = np.empty((len(rho_grid),12))
Fh_rate_net_vint_open_mm_int = np.empty((len(rho_grid),12))


for irho in np.arange(0,len(rho_grid)):   
    for mm in np.arange(0,12):
        
        Fs_rate_sfc_open_mm_int[irho,mm] = np.nansum(np.nansum(Fs_rate_delta_sfc_open_mm[mm,irho,:], axis=1), axis=0)
        Ff_rate_sfc_open_mm_int[irho,mm] = np.nansum(np.nansum(Ff_rate_delta_sfc_open_mm[mm,irho,:], axis=1), axis=0)
        Fm_rate_sfc_open_mm_int[irho,mm] = np.nansum(np.nansum(Fm_rate_delta_sfc_open_mm[mm,irho,:], axis=1), axis=0)
        Fh_rate_sfc_open_mm_int[irho,mm] = np.nansum(np.nansum(Fh_rate_delta_sfc_open_mm[mm,irho,:], axis=1), axis=0)
        
        Fs_rate_adv_vint_open_mm_int[irho,mm] = np.nansum(np.nansum(Fs_rate_delta_adv_vint_open_mm[mm,irho,:], axis=1), axis=0)
        Ff_rate_adv_vint_open_mm_int[irho,mm] = np.nansum(np.nansum(Ff_rate_delta_adv_vint_open_mm[mm,irho,:], axis=1), axis=0)
        Fh_rate_adv_vint_open_mm_int[irho,mm] = np.nansum(np.nansum(Fh_rate_delta_adv_vint_open_mm[mm,irho,:], axis=1), axis=0)
        
        Fs_rate_diff_vint_open_mm_int[irho,mm] = np.nansum(np.nansum(Fs_rate_delta_diff_vint_open_mm[mm,irho,:], axis=1), axis=0)
        Ff_rate_diff_vint_open_mm_int[irho,mm] = np.nansum(np.nansum(Ff_rate_delta_diff_vint_open_mm[mm,irho,:], axis=1), axis=0)
        Fh_rate_diff_vint_open_mm_int[irho,mm] = np.nansum(np.nansum(Fh_rate_delta_diff_vint_open_mm[mm,irho,:], axis=1), axis=0)
        
        Fs_rate_net_vint_open_mm_int[irho,mm] = np.nansum(np.nansum(Fs_rate_delta_net_vint_open_mm[mm,irho,:], axis=1), axis=0)
        Ff_rate_net_vint_open_mm_int[irho,mm] = np.nansum(np.nansum(Ff_rate_delta_net_vint_open_mm[mm,irho,:], axis=1), axis=0)
        Fh_rate_net_vint_open_mm_int[irho,mm] = np.nansum(np.nansum(Fh_rate_delta_net_vint_open_mm[mm,irho,:], axis=1), axis=0)
...
Fm_sig_sfc_shelf =  np.nanmean(Fm_sig_sfc_shelf_mm, axis=1)
Fh_sig_sfc_shelf =  np.nanmean(Fh_sig_sfc_shelf_mm, axis=1)
F_sig_sfc_shelf = -Fs_sig_sfc_shelf + Fh_sig_sfc_shelf + Fm_sig_sfc_shelf

# SFC: open ocean
Fs_sig_sfc_open_mm = -Fs_rate_sfc_open_mm_int*rho0/1e6
Fm_sig_sfc_open_mm = -Fm_rate_sfc_open_mm_int*rho0/1e6 
Fh_sig_sfc_open_mm = -Fh_rate_sfc_open_mm_int*rho0/1e6 
F_sig_sfc_open_mm = -Fs_sig_sfc_open_mm + Fh_sig_sfc_open_mm + Fm_sig_sfc_open_mm
# - calculate the anual average of the monthly ars:
Fs_sig_sfc_open =  np.nanmean(Fs_sig_sfc_open_mm, axis=1)
Fm_sig_sfc_open =  np.nanmean(Fm_sig_sfc_open_mm, axis=1)
Fh_sig_sfc_open =  np.nanmean(Fh_sig_sfc_open_mm, axis=1)
F_sig_sfc_open = -Fs_sig_sfc_open + Fh_sig_sfc_open + Fm_sig_sfc_open
...
cf=plt.plot(rho_grid,Fm_sig_sfc_shelf,'-g',label='WMT due to sfc ice melt')
...
plt.title('Open-ocean')
cf=plt.plot(rho_grid,-Fs_sig_sfc_open,'--b',label='WMT due to sfc salt flux')
cf=plt.plot(rho_grid,Fm_sig_sfc_open,'-g',label='WMT due to sfc ice melt')
ch=plt.plot(rho_grid,Fh_sig_sfc_open,'r',label='WMT due to sfc heat flux')
ct=plt.plot(rho_grid,F_sig_sfc_open,'k',label='WMT due to sfc total flux')
plt.legend()
plt.xlim(26.5,28),plt.ylim(-16,2.5)
plt.grid(True)

name_fig="waom10extend_shflim_S_0.25Q_WMT_sfc_fluxes_annual_shelf_open_comparison.png"
...
name_fig="waom10extend_shflim_S_0.25Q_WMT_heat-salt_vint_annual_shelf.png"
plt.savefig(fig_path + name_fig, dpi=300)

# ADV (vint)
Fs_sig_adv_vint_open_mm = -Fs_rate_adv_vint_open_mm_int*rho0/1e6 
Fh_sig_adv_vint_open_mm = -Fh_rate_adv_vint_open_mm_int*rho0/1e6 
F_sig_adv_vint_open_mm = -Fs_sig_adv_vint_open_mm + Fh_sig_adv_vint_open_mm
# - calculate the anual average of the monthly ars:
Fs_sig_adv_vint_open =  np.nanmean(Fs_sig_adv_vint_open_mm, axis=1)
Fh_sig_adv_vint_open =  np.nanmean(Fh_sig_adv_vint_open_mm, axis=1)
F_sig_adv_vint_open = -Fs_sig_adv_vint_open + Fh_sig_adv_vint_open

# DIFF
Fs_sig_diff_vint_open_mm = -Fs_rate_diff_vint_open_mm_int*rho0/1e6 
Fh_sig_diff_vint_open_mm = -Fh_rate_diff_vint_open_mm_int*rho0/1e6 
F_sig_diff_vint_open_mm = -Fs_sig_diff_vint_open_mm + Fh_sig_diff_vint_open_mm
# - calculate the anual average of the monthly ars:
Fs_sig_diff_vint_open =  np.nanmean(Fs_sig_diff_vint_open_mm, axis=1)
Fh_sig_diff_vint_open =  np.nanmean(Fh_sig_diff_vint_open_mm, axis=1)
F_sig_diff_vin_opent = -Fs_sig_diff_vint_open + Fh_sig_diff_vint_open

# NET
Fs_sig_net_vint_open_mm = -Fs_rate_net_vint_open_mm_int*rho0/1e6 
Fh_sig_net_vint_open_mm = -Fh_rate_net_vint_open_mm_int*rho0/1e6 
F_sig_net_vint_open_mm = -Fs_sig_net_vint_open_mm + Fh_sig_net_vint_open_mm
# - calculate the anual average of the monthly ars:
Fs_sig_net_vint_open =  np.nanmean(Fs_sig_net_vint_open_mm, axis=1)
Fh_sig_net_vint_open =  np.nanmean(Fh_sig_net_vint_open_mm, axis=1)
F_sig_net_vint_open = -Fs_sig_net_vint_open + Fh_sig_net_vint_open
...
ax1 = fig.add_subplot(222)
plt.title('Salt budget')
cs=plt.plot(rho_grid,-Fs_sig_net_vint_open,'b',label='WMT due to salt tend')
cs=plt.plot(rho_grid,-Fs_sig_diff_vint_open,'--b',label='WMT due to salt diff')
cs=plt.plot(rho_grid,-Fs_sig_adv_vint_open,':b',label='WMT due to salt adv')
plt.legend()
plt.ylabel('Transformation rate (Sv)')
...
ax1 = fig.add_subplot(223)
plt.title('Heat budget')
ch=plt.plot(rho_grid,Fh_sig_net_vint_open,'r',label='WMT due to heat tend')
ch=plt.plot(rho_grid,Fh_sig_diff_vint_open,'--r',label='WMT due to heat diff')
ch=plt.plot(rho_grid,Fh_sig_adv_vint_open,':r',label='WMT due to heat adv')
plt.legend()
plt.ylabel('Transformation rate (Sv)')
plt.xlim(26.5,28),plt.ylim(-2.5,2.5)
plt.grid(True)
...
ax2 = fig.add_subplot(221)
plt.title('Surface fluxes')
cs=plt.plot(rho_grid,-Fs_sig_sfc_open,'b',label='WMT due to salt sfc')
cf=plt.plot(rho_grid,Fm_sig_sfc_open,'-g',label='WMT due to sfc ice melt')
ch=plt.plot(rho_grid,Fh_sig_sfc_open,'r',label='WMT due to heat sfc')
ct=plt.plot(rho_grid,F_sig_sfc_open,'k',label='WMT due to total sfc')
plt.legend()
plt.ylabel('Transformation rate (Sv)')
plt.xlim(26.5,28),plt.ylim(-2.5,2.5)
plt.grid(True)
...
ax2 = fig.add_subplot(224)
plt.title('Net tendencies')
cs=plt.plot(rho_grid,-Fs_sig_net_vint_open,'-b',label='WMT tendency due to salt sfc')
ch=plt.plot(rho_grid,Fh_sig_net_vint_open,'-r',label='WMT tendency due to heat sfc')
ct=plt.plot(rho_grid,Fh_sig_net_vint_open-Fs_sig_net_vint_open,'k',label='WMT tendency total')
plt.legend()
plt.ylabel('Transformation rate (Sv)')
plt.xlim(26.5,28),plt.ylim(-2.5,2.5)
plt.grid(True)

name_fig="waom10extend_shflim_S_0.25Q_WMT_heat-salt_vint_annual_open.png"
plt.savefig(fig_path + name_fig, dpi=300)
...
# call cartopy projection
    ...
    cy=plt.pcolormesh(lon_rho,lat_rho,np.nanmean(Fs_rate_delta_net_vint_shelf_mm[:,irho,:], axis=0)*rho0/(dx*dy), transform=ccrs.PlateCarree(), cmap=plt.cm.coolwarm)
    plt.colorbar(cy)
    ...
    cy=plt.pcolormesh(lon_rho,lat_rho,np.nanmean(Fh_rate_delta_net_vint_shelf_mm[:,irho,:], axis=0)*rho0/(dx*dy), transform=ccrs.PlateCarree(), cmap=plt.cm.coolwarm)
    plt.colorbar(cy)
    ...
    name_fig="waom10extend_shflim_S_0.25Q_WMTmaps_annual_yr20_l" + str(irho)  + "_shelf.png"
    plt.savefig(fig_path + name_fig, dpi=300)
    plt.close()
...
#Fs_rate_mm_int[irho,mm] = np.nansum(np.nansum(Fs_rate_delta_mm[mm,irho,:], axis=1), axis=0)*rho0/0.1
#Fh_rate_mm_int[irho,mm] = np.nansum(np.nansum(Fh_rate_delta_mm[mm,irho,:], axis=1), axis=0)*rho0/0.1
print(Fs_rate_delta_net_vint_open_mm.shape)
...
# call cartopy projection
    ...
    ax2 = fig.add_subplot(222, projection=proj)
    plt.title('FWF, $\sigma_{\Theta}$ = ' + str(rho_grid[irho]))
    cy=plt.pcolormesh(lon_rho,lat_rho,np.nanmean(Fs_rate_delta_net_vint_open_mm[:,irho,:], axis=0)*rho0/(dx*dy), transform=ccrs.PlateCarree(), cmap=plt.cm.coolwarm)
    plt.colorbar(cy)
    ax2.gridlines() # draw_labels=True,linewidth=2, color='white', alpha=0.5, linestyle='--')
    lonlat_labels(ax2)
    ax2.set_extent([-180, 180, -90, -60], crs=ccrs.PlateCarree())
    ...
    ax3 = fig.add_subplot(223, projection=proj)
    plt.title('HF, $\sigma_{\Theta}$ = ' + str(rho_grid[irho]))
    cy=plt.pcolormesh(lon_rho,lat_rho,np.nanmean(Fh_rate_delta_net_vint_open_mm[:,irho,:], axis=0)*rho0/(dx*dy), transform=ccrs.PlateCarree(), cmap=plt.cm.coolwarm)
    plt.colorbar(cy)
    ax3.gridlines() # draw_labels=True,linewidth=2, color='white', alpha=0.5, linestyle='--')
    lonlat_labels(ax3)
    ax3.set_extent([-180, 180, -90, -60], crs=ccrs.PlateCarree())
    ...
    ax4 = fig.add_subplot(224, projection=proj)
    plt.title('Total, $\sigma_{\Theta}$ = ' + str(rho_grid[irho]))
    cy=plt.pcolormesh(lon_rho,lat_rho,np.nanmean(Fh_rate_delta_net_vint_open_mm[:,irho,:], axis=0)*rho0/(dx*dy)+np.nanmean(Fs_rate_delta_net_vint_open_mm[:,irho,:], axis=0)*rho0/(dx*dy), transform=ccrs.PlateCarree(), cmap=plt.cm.coolwarm)
    plt.colorbar(cy)
    ax4.gridlines() # draw_labels=True,linewidth=2, color='white', alpha=0.5, linestyle='--')
    lonlat_labels(ax4)
    ax4.set_extent([-180, 180, -90, -60], crs=ccrs.PlateCarree())
    ...
    name_fig="waom10extend_shflim_S_0.25Q_WMTmaps_annual_yr20_l" + str(irho)  + "_open.png"
    plt.savefig(fig_path + name_fig, dpi=300)
    plt.close()
...
name_fig="waom10extend_shflim_S_0.25Q_WMTmaps_annual_yr20_shelf.png"
plt.savefig(fig_path + name_fig, dpi=300)
plt.close()
...
name_fig="waom10extend_shflim_S_0.25Q_WMTmaps_annual_yr20_open.png"
plt.savefig(fig_path + name_fig, dpi=300)
plt.close()
