...
Fs_rate_delta_adv_vint_iceshelf_mm = wmt(salt_adv_mld_vint*mask_shelf*mask_outice, dx, dy,'budget')
Fs_rate_delta_diff_vint_iceshelf_mm = wmt(R_s_vint*mask_shelf*mask_outice, dx, dy,'budget')
Fs_rate_delta_net_vint_iceshelf_mm = wmt(salt_net_mld_vint*mask_shelf*mask_outice, dx, dy,'budget')
Fs_rate_delta_sfc_iceshelf_mm = wmt(salt_sfc*mask_shelf*mask_outice, dx, dy,'sfc_frc')

Fh_rate_delta_adv_vint_iceshelf_mm = wmt(temp_adv_mld_vint*mask_shelf*mask_outice, dx, dy,'budget')
Fh_rate_delta_diff_vint_iceshelf_mm = wmt(R_t_vint*mask_shelf*mask_outice, dx, dy,'budget')
Fh_rate_delta_net_vint_iceshelf_mm = wmt(temp_net_mld_vint*mask_shelf*mask_outice, dx, dy,'budget')
...
Fs_rate_sfc_iceshelf_mm_int = np.empty((len(rho_grid),12))
Fh_rate_sfc_iceshelf_mm_int = np.empty((len(rho_grid),12))

Fs_rate_adv_vint_iceshelf_mm_int = np.empty((len(rho_grid),12))
Fh_rate_adv_vint_iceshelf_mm_int = np.empty((len(rho_grid),12))
Fs_rate_diff_vint_iceshelf_mm_int = np.empty((len(rho_grid),12))
Fh_rate_diff_vint_iceshelf_mm_int = np.empty((len(rho_grid),12))
...
for irho in np.arange(0,len(rho_grid)):   
    for mm in np.arange(0,12):
        ...
        Fs_rate_sfc_iceshelf_mm_int[irho,mm] = np.nansum(np.nansum(Fs_rate_delta_sfc_iceshelf_mm[mm,irho,:], axis=1), axis=0)
        Fh_rate_sfc_iceshelf_mm_int[irho,mm] = np.nansum(np.nansum(Fh_rate_delta_sfc_iceshelf_mm[mm,irho,:], axis=1), axis=0)
...
Fs_sig_sfc_iceshelf_mm = -Fs_rate_sfc_iceshelf_mm_int*Dt/1e6
Fh_sig_sfc_iceshelf_mm = -Fh_rate_sfc_iceshelf_mm_int*Dt/1e6 
F_sig_sfc_iceshelf_mm = -Fs_sig_sfc_iceshelf_mm + Fh_sig_sfc_iceshelf_mm
# - calculate the anual average of the monthly ars:
Fs_sig_sfc_iceshelf =  np.nanmean(Fs_sig_sfc_iceshelf_mm, axis=1)
Fh_sig_sfc_iceshelf =  np.nanmean(Fh_sig_sfc_iceshelf_mm, axis=1)
F_sig_sfc_iceshelf = -Fs_sig_sfc_iceshelf + Fh_sig_sfc_iceshelf
...
cf=plt.plot(rho_grid,-Fs_sig_sfc_iceshelf,'--b',label='WMT due to sfc salt flux')
...
name_fig="waom4extend_shflim_S_0.25Q_10km-bathy_WMT_sfc_fluxes_annual_iceshelf_comparison.png"
plt.savefig(fig_path + name_fig, dpi=300)

# ADV (vint)
Fs_sig_adv_vint_iceshelf_mm = -Fs_rate_adv_vint_iceshelf_mm_int*Dt/1e6 
Fh_sig_adv_vint_iceshelf_mm = -Fh_rate_adv_vint_iceshelf_mm_int*Dt/1e6 
F_sig_adv_vint_iceshelf_mm = -Fs_sig_adv_vint_iceshelf_mm + Fh_sig_adv_vint_iceshelf_mm
# - calculate the anual average of the monthly ars:
Fs_sig_adv_vint_iceshelf =  np.nanmean(Fs_sig_adv_vint_iceshelf_mm, axis=1)
...
F_sig_diff_vint_iceshelf_mm = -Fs_sig_diff_vint_iceshelf_mm + Fh_sig_diff_vint_iceshelf_mm
# - calculate the anual average of the monthly ars:
...
Fh_sig_net_vint_iceshelf_mm = -Fh_rate_net_vint_iceshelf_mm_int*Dt/1e6 
F_sig_net_vint_iceshelf_mm = -Fs_sig_net_vint_iceshelf_mm + Fh_sig_net_vint_iceshelf_mm
# - calculate the anual average of the monthly ars:
Fs_sig_net_vint_iceshelf =  np.nanmean(Fs_sig_net_vint_iceshelf_mm, axis=1)
Fh_sig_net_vint_iceshelf =  np.nanmean(Fh_sig_net_vint_iceshelf_mm, axis=1)
F_sig_net_vint_iceshelf = -Fs_sig_net_vint_iceshelf + Fh_sig_net_vint_iceshelf

fig = plt.figure(figsize=(16,10))
ax1 = fig.add_subplot(222)
plt.title('Salt budget')
cs=plt.plot(rho_grid,-Fs_sig_net_vint_iceshelf,'b',label='WMT due to salt tend')
cs=plt.plot(rho_grid,-Fs_sig_diff_vint_iceshelf,'--b',label='WMT due to salt diff')
cs=plt.plot(rho_grid,-Fs_sig_adv_vint_iceshelf,':b',label='WMT due to salt adv')
plt.legend()
...
ch=plt.plot(rho_grid,Fh_sig_net_vint_iceshelf,'r',label='WMT due to heat tend')
ch=plt.plot(rho_grid,Fh_sig_diff_vint_iceshelf,'--r',label='WMT due to heat diff')
ch=plt.plot(rho_grid,Fh_sig_adv_vint_iceshelf,':r',label='WMT due to heat adv')
plt.legend()
plt.ylabel('Transformation rate (Sv)')
plt.xlim(26.5,28),plt.ylim(-5,5)
plt.grid(True)
...
cs=plt.plot(rho_grid,-Fs_sig_sfc_iceshelf,'b',label='WMT due to salt sfc')
ch=plt.plot(rho_grid,Fh_sig_sfc_iceshelf,'r',label='WMT due to heat sfc')
ct=plt.plot(rho_grid,F_sig_sfc_iceshelf,'k',label='WMT due to total sfc')
plt.legend()
plt.ylabel('Transformation rate (Sv)')
plt.xlim(26.5,28),plt.ylim(-5,5)
plt.grid(True)
...
cs=plt.plot(rho_grid,-Fs_sig_net_vint_iceshelf,'-b',label='WMT tendency due to salt sfc')
ch=plt.plot(rho_grid,Fh_sig_net_vint_iceshelf,'-r',label='WMT tendency due to heat sfc')
ct=plt.plot(rho_grid,Fh_sig_net_vint_iceshelf-Fs_sig_net_vint_iceshelf,'k',label='WMT tendency total')
#plt.legend()
plt.ylabel('Transformation rate (Sv)')
plt.xlim(26.5,28),plt.ylim(-5,5)
plt.grid(True)
...
name_fig="waom4extend_shflim_S_0.25Q_10km-bathy_WMT_heat-salt_vint_annual_iceshelf.png"
plt.savefig(fig_path + name_fig, dpi=300)
...
print(Fs_rate_delta_net_vint_iceshelf_mm.shape)
...
# call cartopy projection
    ...
    cy=plt.pcolormesh(lon_rho,lat_rho,np.nanmean(Fs_rate_delta_net_vint_iceshelf_mm[:,irho,:], axis=0)*Dt/(dx*dy), transform=ccrs.PlateCarree(), cmap=plt.cm.coolwarm)
    plt.colorbar(cy)
    ...
    cy=plt.pcolormesh(lon_rho,lat_rho,np.nanmean(Fh_rate_delta_net_vint_iceshelf_mm[:,irho,:], axis=0)*Dt/(dx*dy), transform=ccrs.PlateCarree(), cmap=plt.cm.coolwarm)
    plt.colorbar(cy)
    ...
    name_fig="waom4extend_shflim_S_0.25Q_10km-bathy_WMTmaps_annual_yr10_l" + str(irho)  + "_iceshelf.png"
    plt.savefig(fig_path + name_fig, dpi=300)
    plt.close()
...
name_fig="waom4extend_shflim_S_0.25Q_10km-bathy_WMTmaps_annual_yr10_iceshelf.png"
plt.savefig(fig_path + name_fig, dpi=300)
plt.close()
...
np.savez(npy_path + 'WAOM4extend_10km-bathy_WMT_iceshelf_sfc_total',rho_grid = rho_grid, F_sig_sfc_iceshelf = F_sig_sfc_iceshelf)
np.savez(npy_path + 'WAOM4extend_10km-bathy_WMT_iceshelf_sfc_salt',rho_grid = rho_grid, Fs_sig_sfc_iceshelf = Fs_sig_sfc_iceshelf)
np.savez(npy_path + 'WAOM4extend_10km-bathy_WMT_iceshelf_sfc_heat',rho_grid = rho_grid, Fh_sig_sfc_iceshelf = Fh_sig_sfc_iceshelf)

np.savez(npy_path + 'WAOM4extend_10km-bathy_WMT_iceshelf_vint_salt_net',rho_grid = rho_grid, Fs_sig_net_vint_iceshelf = Fs_sig_net_vint_iceshelf)
np.savez(npy_path + 'WAOM4extend_10km-bathy_WMT_iceshelf_vint_salt_diff',rho_grid = rho_grid, Fs_sig_diff_vint_iceshelf = Fs_sig_diff_vint_iceshelf)
np.savez(npy_path + 'WAOM4extend_10km-bathy_WMT_iceshelf_vint_salt_adv',rho_grid = rho_grid, Fs_sig_adv_vint_iceshelf = Fs_sig_adv_vint_iceshelf)

np.savez(npy_path + 'WAOM4extend_10km-bathy_WMT_iceshelf_vint_heat_net', rho_grid = rho_grid, Fh_sig_net_vint_iceshelf = Fh_sig_net_vint_iceshelf)
np.savez(npy_path + 'WAOM4extend_10km-bathy_WMT_iceshelf_vint_heat_diff', rho_grid = rho_grid, Fh_sig_diff_vint_iceshelf = Fh_sig_diff_vint_iceshelf)
np.savez(npy_path + 'WAOM4extend_10km-bathy_WMT_iceshelf_vint_heat_adv', rho_grid = rho_grid, Fh_sig_adv_vint_iceshelf = Fh_sig_adv_vint_iceshelf)
