...
#    salt_net_tmp = np.nanmean(Fs_rate_delta_net_vint_iceshelf_mm[:,irho,:], axis=0)*rho0/(dx*dy)
    ...
    m_sfc_tmp = np.empty((12,1400,1575))
    for mm in np.arange(0,12):
...
ax1.gridlines()
lonlat_labels(ax1)
ax1.set_extent([-80, 10, -80, -65], crs=ccrs.PlateCarree())
...
ax2.gridlines()
lonlat_labels(ax2)
ax2.set_extent([-80, 10, -80, -65], crs=ccrs.PlateCarree())
...
ax3.gridlines()
lonlat_labels(ax3)
ax3.set_extent([-80, 10, -80, -65], crs=ccrs.PlateCarree())
...
name_fig="waom4extend_shflim_S_0.25Q_WMTmaps_salt_budget_annual_yr10_iceshelf_RFIS.png"
...
name_fig="waom4extend_shflim_S_0.25Q_WMTmaps_salt_budget_annual_yr20_basalmelt_iceshelf_RFIS.png"
plt.savefig(fig_path + name_fig, dpi=300)
plt.close()
...
#    plt.clim(-5e-6,5e-6)
    ...
    name_fig="waom4extend_shflim_S_0.25Q_WMTmaps_salt_budget_annual_yr10_iceshelf_mm=" + str(mm) + "_RFIS.png"
    plt.savefig(fig_path + name_fig, dpi=300)
    plt.close()
