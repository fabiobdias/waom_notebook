...
# using xr.open_mfdataset
    ...
    # define masks for cavities less_equal than 400m & more than 400m ice draft depth (to separate modes into shallow and deep):

    msk_ice_deep = ma.masked_where(ice_draft>=-400, np.ones(ice_draft.shape))
    msk_ice_shallow = ma.masked_where(ice_draft<-400, np.ones(ice_draft.shape))
    ...
    files_path = '/g/data/hh5/tmp/access-om/fbd581/ROMS/postprocessing/cross_contour_tmp/'
    melt_bins_shallow_xr.to_netcdf(files_path + 'WAOM4_OHB_lonlatbins-v2_melt_shallow_daily_v2_400m.nc', mode='w', format="NETCDF4")
    melt_bins_deep_xr.to_netcdf(files_path + 'WAOM4_OHB_lonlatbins-v2_melt_deep_daily_v2_400m.nc', mode='w', format="NETCDF4")
