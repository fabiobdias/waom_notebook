...
# using xr.open_mfdataset
    ...
    # define masks for cavities less_equal than 150m & more than 150m ice draft depth (to separate modes into shallow and deep):

    msk_ice_deep = ma.masked_where(ice_draft>=-150, np.ones(ice_draft.shape))
    msk_ice_shallow = ma.masked_where(ice_draft<-150, np.ones(ice_draft.shape))
    ...
    melt_bins_shallow = np.empty((365,127))
    melt_bins_deep = np.empty((365,127))
    for ll in np.arange(127):
        ...
        for tt in np.arange(365):
            ...
            melt_bins_shallow[tt,ll] = np.divide(melt_area_avg_s,area_sum_masked_s)
            ...
            melt_bins_deep[tt,ll] = np.divide(melt_area_avg_d,area_sum_masked_d)
    ...
    files_path = '/g/data/hh5/tmp/access-om/fbd581/ROMS/postprocessing/cross_contour_tmp/'
    melt_bins_shallow_xr.to_netcdf(files_path + 'WAOM4_OHB_lonlatbins_melt_shallow_daily_v4', mode='w', format="NETCDF4")
    melt_bins_deep_xr.to_netcdf(files_path + 'WAOM4_OHB_lonlatbins_melt_deep_daily_v4', mode='w', format="NETCDF4")
