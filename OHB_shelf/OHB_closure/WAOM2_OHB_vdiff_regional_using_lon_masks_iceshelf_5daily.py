...
# - WAOM2extend_shflim_S_0.25Q_OHB_shelf_budget_closure.ipynb
...
if __name__== '__main__':
    ...
    # save to netcdf file:
    coordinatesC=dict(ocean_time=months, s_rho=(['s_rho'], np.arange(0,31)), eta_rho=(['eta_rho'], np.arange(0,2800)), xi_rho=(['xi_rho'], np.arange(0,3150)))
    ...
    dm = xr.open_dataset('/g/data/hh5/tmp/access-om/fbd581/ROMS/postprocessing/cross_contour_tmp/WAOM2_masked_lonBins.nc')
    ...
    temp_vdiff_int = np.empty((len(mask_LonBins),tlen))
    ...
    for ll in np.arange(0,len(mask_LonBins)):
        ...
        for mm in np.arange(0,tlen):
            print('debuging, print mm, ll, temp_vdiff.shape, dz.shape =',
                    mm, ll, temp_vdiff.shape, dz.shape)
        # - multplying by dz:
            temp_vdiff_dz = temp_vdiff[mm,:]*dz[mm,:]
            temp_vdiff_vint = np.nansum(temp_vdiff_dz, axis=0)
            temp_vdiff_vint_masked =  ma.masked_where(condition1, temp_vdiff_vint)
            temp_vdiff_vol = temp_vdiff_vint_masked*area
            temp_vdiff_int[ll,mm] = np.nansum(np.nansum(temp_vdiff_vol,axis=1), axis=0)*Cp*rho0
            del temp_vdiff_dz, temp_vdiff_vint, temp_vdiff_vint_masked, temp_vdiff_vol
    ...
    temp_vdiff_int_xr = xr.DataArray(temp_vdiff_int, coords = coordinatesC, dims = ['lon_bins','ocean_time'])

    files_path = '/g/data/hh5/tmp/access-om/fbd581/ROMS/postprocessing/cross_contour_tmp/'
    temp_vdiff_int_xr.to_netcdf(files_path + 'WAOM2_OHB_lonbins_temp_vdiff_vint_5daily_iceshelf', mode='w', format="NETCDF4")
