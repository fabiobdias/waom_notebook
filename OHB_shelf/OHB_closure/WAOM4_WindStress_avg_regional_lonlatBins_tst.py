...
# using xr.open_mfdataset
    ...
    print('Generated WS_masked ok, checking it:', WS_masked)

    wind_str_bins = np.empty((365,127))
    for ll in np.arange(5):#127):
...
#    months=np.arange(0,365)*(1/30.41667)
    months=np.arange(0,30)*(1/30.41667)
    lon_bin_midpts = np.arange(-178.5,180,3)
    ...
    files_path = '/g/data/hh5/tmp/access-om/fbd581/ROMS/postprocessing/cross_contour_tmp/'
    wind_str_bins_xr.to_netcdf(files_path + 'WAOM4_OHB_lonlatbins_wind_str_daily_tst', mode='w', format="NETCDF4")
