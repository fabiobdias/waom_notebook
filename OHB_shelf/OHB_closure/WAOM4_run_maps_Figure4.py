...
if __name__== '__main__':
    ...
    for dd in np.arange(0,2):
        ...
        for rr in range(0,12):
            ...
            sum_T = (temp3d_4km.isel(ocean_time=sum_MM)+273.15) - (temp3d_4km.mean('ocean_time')+273.15) #  Jan - Annual (+ means summer is warmer than mean)
            ...
            win_T = (temp3d_4km.isel(ocean_time=win_MM)+273.15) - (temp3d_4km.mean('ocean_time')+273.15) # Jul - Anual (- mean winter is colder...)
            ...
            # define february monthly average: Getz:
            # if rr==9: #5914.0 59.0
            #     S_S = salt_4km_CF.isel(contour_index_array=slice(int(indexes_subr[rr,0]),-1)).isel(contour_index_array=slice(0,int(indexes_subr[rr+1,1]))).isel(ocean_time=dayofmonth[sum_MM]).mean('ocean_time')
            #     T_S = temp_4km_CF.isel(contour_index_array=slice(int(indexes_subr[rr,0]),-1)).isel(contour_index_array=slice(0,int(indexes_subr[rr+1,1]))).isel(ocean_time=dayofmonth[sum_MM]).mean('ocean_time')
            #     Z_S = -z_rho_4km_CF.isel(contour_index_array=slice(int(indexes_subr[rr,0]),-1)).isel(contour_index_array=slice(0,int(indexes_subr[rr+1,1]))).mean('ocean_time')
            #     # define august monthly average:
            #     S_W = salt_4km_CF.isel(contour_index_array=slice(int(indexes_subr[rr,0]),-1)).isel(contour_index_array=slice(0,int(indexes_subr[rr+1,1]))).isel(ocean_time=dayofmonth[win_MM]).mean('ocean_time')
            #     T_W = temp_4km_CF.isel(contour_index_array=slice(int(indexes_subr[rr,0]),-1)).isel(contour_index_array=slice(0,int(indexes_subr[rr+1,1]))).isel(ocean_time=dayofmonth[win_MM]).mean('ocean_time')
            #     Z_W = -z_rho_4km_CF.isel(contour_index_array=slice(int(indexes_subr[rr,0]),-1)).isel(contour_index_array=slice(0,int(indexes_subr[rr+1,1]))).mean('ocean_time')
            # else:
            #     S_S = salt_4km_CF.isel(contour_index_array=slice(int(indexes_subr[rr,0]),int(indexes_subr[rr,1]))).isel(ocean_time=dayofmonth[sum_MM]).mean('ocean_time')
            #     T_S = temp_4km_CF.isel(contour_index_array=slice(int(indexes_subr[rr,0]),int(indexes_subr[rr,1]))).isel(ocean_time=dayofmonth[sum_MM]).mean('ocean_time')
            #     Z_S = -z_rho_4km_CF.isel(contour_index_array=slice(int(indexes_subr[rr,0]),int(indexes_subr[rr,1]))).isel(ocean_time=dayofmonth[sum_MM]).mean('ocean_time')
            #     # define august monthly average:
            #     S_W = salt_4km_CF.isel(contour_index_array=slice(int(indexes_subr[rr,0]),int(indexes_subr[rr,1]))).isel(ocean_time=dayofmonth[win_MM]).mean('ocean_time')
            #     T_W = temp_4km_CF.isel(contour_index_array=slice(int(indexes_subr[rr,0]),int(indexes_subr[rr,1]))).isel(ocean_time=dayofmonth[win_MM]).mean('ocean_time')
            #     Z_W = -z_rho_4km_CF.isel(contour_index_array=slice(int(indexes_subr[rr,0]),int(indexes_subr[rr,1]))).isel(ocean_time=dayofmonth[win_MM]).mean('ocean_time')
            Mask_tmp_subr = mask_LonBins_4km.isel(lon_bin=slice(sind_subreg[rr],eind_subreg[rr])).sum('lon_bin')*w4_mask_zice*w4_mask_land*w4_mask_shelf
            ...
            name_fig='waom4_MapsT+SHF_season_' + pname[dd] + '_' + Pname +'.png'
