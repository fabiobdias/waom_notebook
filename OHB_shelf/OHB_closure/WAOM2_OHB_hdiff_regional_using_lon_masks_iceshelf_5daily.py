...
if __name__== '__main__':
    ...
    temp_hdiff_int = np.empty((len(mask_LonBins),tlen))
    ...
    for ll in np.arange(0,len(mask_LonBins)):
        ...
        for mm in np.arange(0,tlen):
            print('debuging, print mm, ll, temp_hdiff.shape, dz.shape =',
                    mm, ll, temp_hdiff.shape, dz.shape)
        # - multplying by dz:
            temp_hdiff_dz = temp_hdiff[mm,:]*dz[mm,:]
            temp_hdiff_vint = np.nansum(temp_hdiff_dz, axis=0)
            temp_hdiff_vint_masked =  ma.masked_where(condition1, temp_hdiff_vint)
            temp_hdiff_vol = temp_hdiff_vint_masked*area
            temp_hdiff_int[ll,mm] = np.nansum(np.nansum(temp_hdiff_vol,axis=1), axis=0)*Cp*rho0
            del temp_hdiff_dz, temp_hdiff_vint, temp_hdiff_vint_masked, temp_hdiff_vol
    ...
    temp_hdiff_int_xr = xr.DataArray(temp_hdiff_int, coords = coordinatesC, dims = ['lon_bins','ocean_time'])

    files_path = '/g/data/hh5/tmp/access-om/fbd581/ROMS/postprocessing/cross_contour_tmp/'
    temp_hdiff_int_xr.to_netcdf(files_path + 'WAOM2_OHB_lonbins_temp_hdiff_vint_5daily_iceshelf', mode='w', format="NETCDF4")
