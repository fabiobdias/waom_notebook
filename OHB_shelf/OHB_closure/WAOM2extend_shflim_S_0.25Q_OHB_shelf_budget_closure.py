...
if __name__== '__main__':
    ...
    ds = xr.open_mfdataset(paths="/g/data/hh5/tmp/access-om/fbd581/ROMS/OUTPUT/waom2extend_shflim_S_0.25Q/output_yr5_diag/ocean_avg_00*.nc" , chunks={'eta_rho': '200MB'}, parallel=True, drop_variables=vars2drop, decode_times=False) # , concat_dim="ocean_time"
    ...
    ds = xr.open_mfdataset(paths="/g/data/hh5/tmp/access-om/fbd581/ROMS/OUTPUT/waom2extend_shflim_S_0.25Q/output_yr5_diag/ocean_his_00*.nc" , chunks={'eta_rho': '200MB'}, parallel=True, decode_times=False) #, chuncks="auto", concat_dim="ocean_time"
    
    #- preserving 5-days avgs
    temp_snap = ds.temp.drop_isel(ocean_time=[17,36,44,55]) # dropping indexes to match a bit better AVG DTs; temp solution. Permanent one involves re-running models with exact HIS/AVG DTs...
    ds.close()
    print('temp_snap shape (just after reading it) = ', temp_snap.shape)
    print(temp_snap)
    ...
    ds = xr.open_mfdataset('/g/data/hh5/tmp/access-om/fbd581/ROMS/OUTPUT/waom2extend_shflim_S_0.25Q/output_yr4b/ocean_rst.nc')
    #temp_ini = ds.variables["temp"].isel(ocean_time=1, two=0)
    temp_ini = ds.temp.isel(two=0)
    ds.close()

    # remove first indice of the restart file (not needed):
    temp_ini = temp_ini.drop_isel(ocean_time=[0])
    print('temp_ini shape = ', temp_ini.shape)  
    ...
    # extra steps for temp_ini:
    # 1. get s_rho from temp_snap
    ...
    print('temp_snap and dT (just after concat[temp_ini,temp_snap]', temp_snap.shape, dT.shape)
    ...
    print('temp_Rate shape (just after transpose) = ', temp_Rate.shape)
    
    # calculate surface sigma_theta (potential density)
    sigma_t_sfc = gsw.rho(salt[:,-1,:,:],temp[:,-1,:,:],0) - 1000
    
    # load ice draft to create masks
    di = xr.open_dataset('/g/data/hh5/tmp/access-om/fbd581/ROMS/OUTPUT/waom2extend_shflim_S_0.25Q/output_yr5_diag/ocean_avg_0001.nc')
    ...
    print('TLEN before loop that integrate horizontally shflux/temp_rate:', tlen)
    ...
    name_fig='WAOM2_OHB_south1500m_temp_rate_vint+shflux_maps_annual.png'
    ...
    files_path = '/g/data/hh5/tmp/access-om/fbd581/ROMS/postprocessing/cross_contour_tmp/'
    temp_rate_int_xr.to_netcdf(files_path + 'WAOM2_OHB_1500m_temp_rate_vint_5daily', mode='w', format="NETCDF4")
    shflux_int_xr.to_netcdf(files_path + 'WAOM2_OHB_1500m_shflux_vint_5daily', mode='w', format="NETCDF4")
    ...
    name_fig='WAOM2_OHB_south1500m_vint_annual.png'
    plt.savefig(fig_path + name_fig, dpi=300)
