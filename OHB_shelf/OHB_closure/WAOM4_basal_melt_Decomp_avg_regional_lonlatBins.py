...
# using xr.open_mfdataset
    ...
    msk_ice_deep = ma.masked_where(ice_draft>=-500, np.ones(ice_draft.shape))
    msk_ice_shallow = ma.masked_where(ice_draft<-500, np.ones(ice_draft.shape))
    ...
    files_path = '/g/data/hh5/tmp/access-om/fbd581/ROMS/postprocessing/cross_contour_tmp/'
    melt_bins_shallow_xr.to_netcdf(files_path + 'WAOM4_OHB_lonlatbins_melt_shallow_daily_v2', mode='w', format="NETCDF4")
    melt_bins_deep_xr.to_netcdf(files_path + 'WAOM4_OHB_lonlatbins_melt_deep_daily_v2', mode='w', format="NETCDF4")
