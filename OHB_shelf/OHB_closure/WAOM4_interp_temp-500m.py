...
#
    ...
    for i in range(temp3d_4km.shape[1]):
        for j in range(temp3d_4km.shape[2]):
            print(i,j)
            depth_annual = z_rho3d_4km[:, i, j, :].mean('ocean_time')
            temp_annual = temp3d_4km[:, :, i, j].mean('ocean_time')
            depth_feb = z_rho3d_4km[31:60, i, j, :].mean('ocean_time')
            temp_feb = temp3d_4km[31:60, :, i, j].mean('ocean_time')
            depth_aug = z_rho3d_4km[213:244, i, j, :].mean('ocean_time')
            temp_aug = temp3d_4km[213:244, :, i, j].mean('ocean_time')

            if np.any(np.isnan(depth_annual)) or np.any(np.isnan(temp_annual)):
                print('This i,j is nan!')
                continue  # Skip if there are NaNs in the profile
...
## annual
            ...
            if idx_above == idx_below:
                ...
                temp_above = temp_profile[idx_above]
                temp_below = temp_profile[idx_below]
                interp_temp_annual[i, j] = temp_above + (temp_below - depth_above) * (temp_below - temp_above) / (depth_below - depth_above)
