...
if __name__== '__main__':
    ...
    temp_hadv_int = np.empty((len(mask_LonBins),tlen))
    ...
    for ll in np.arange(0,len(mask_LonBins)):
        ...
        for mm in np.arange(0,tlen):
            print('debuging, print mm, ll, temp_hadv.shape, dz.shape =',
                    mm, ll, temp_hadv.shape, dz.shape)
        # - multplying by dz:
            ...
            temp_hadv_vol = temp_hadv_vint_masked*area
            temp_hadv_int[ll,mm] = np.nansum(np.nansum(temp_hadv_vol,axis=1), axis=0)*Cp*rho0
            del temp_hadv_dz, temp_hadv_vint, temp_hadv_vint_masked, temp_hadv_vol
    ...
    files_path = '/g/data/hh5/tmp/access-om/fbd581/ROMS/postprocessing/cross_contour_tmp/'
    temp_hadv_int_xr.to_netcdf(files_path + 'WAOM2_OHB_lonbins_temp_hadv_vint_5daily_iceshelf', mode='w', format="NETCDF4")
