...
# load contour generated from WAOM4_Extract_1500m_contour.ipynb
    ...
    ds = xr.open_mfdataset(paths="/g/data/hh5/tmp/access-om/fbd581/ROMS/OUTPUT/waom4extend_shflim_S_0.25Q/output_yr10_diag_daily/ocean_avg_00*.nc" , chunks={'eta_rho': '200MB'}, parallel=True, drop_variables=vars2drop, decode_times=False)
    ...
    def extract_transp_across_contour(var_x, var_y):   # var:4D [time,eta_rho,xi_rho]
        ...
        for tt in range(0,tlen): # loop through time
            for zz in range(0,zlen): # loop through z-levels
                ...
                var_x_tmp = var_x[tt,zz,:,:]*mask_x_transport[:,:-1]
                var_y_tmp = var_y[tt,zz,:,:]*mask_y_transport[:-1,:]
    ...
    # extract variables:
    # 1. vol transp
    heat_trans_across_contour = extract_transp_across_contour(HuonT_xr, HvomT_xr)
    ...
    heat_trans_across_contour_xr.to_netcdf(files_path + 'WAOM4_heat_trans_1500m_daily_v3', mode='w', format="NETCDF4")
