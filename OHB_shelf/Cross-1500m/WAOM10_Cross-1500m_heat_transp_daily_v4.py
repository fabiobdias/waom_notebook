...
# load contour generated from WAOM10_Extract_1500m_contour.ipynb
    ...
    # 4) multiply mask_x/y_transport: (do it later like in access-om2; while extracting along contour)
    ...
    def extract_transp_across_contour(var_x, var_y):   # var:4D [time,eta_rho,xi_rho]
        tlen = 1#len(HTu[:,0,0,0])
        zlen = len(HTu[0,:,0,0])
        print(zlen)
        #heat_transp_across_contour = np.empty((tlen,zlen,len(contour_ordering)))
        heat_transp_across_contour = np.empty((zlen,len(contour_ordering)))
        ...
        for zz in range(0,zlen): # loop through z-levels
            ...
            heat_transp_across_contour[zz,:] = heat_transp_across_contour_tmp
    ...
    # extract variables:
    # 1. vol transp
    heat_trans_across_contour = extract_transp_across_contour(HTu.mean('ocean_time'), HTv.mean('ocean_time'))
