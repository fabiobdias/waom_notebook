...
# ----
    u_rho_bot_tmp = np.zeros((len(ds.ocean_time),2800,3150))
    v_rho_bot_tmp = np.zeros((len(ds.ocean_time),2800,3150))
    ubar_rho_tmp = np.zeros((len(ds.ocean_time),2800,3150))
    vbar_rho_tmp = np.zeros((len(ds.ocean_time),2800,3150))
...
# read waom2_swflx file for (E-P fluxes)
dfw = xr.open_dataset("/scratch/project_2000339/boeiradi/waom2_frc/waom2extend_swflux.nc")

dshf = xr.open_dataset("/scratch/project_2000339/boeiradi/waom2_frc/waom2extend_shflux_0.25Q.nc")
...
plt.savefig('/users/boeiradi/COLD_project/postprocessing/figs/WAOM2extend_temp-salt+transects_RF-RIS.png', dpi=300)
plt.close()
...
cv=ax1.quiver(xs[::30,::30],ys[::30,::30],np.nanmean(ubar[:,::30,::30], axis=0),np.nanmean(vbar[:,::30,::30],axis=0), color='gray', transform=src)
ax1.gridlines()
ax1.coastlines(resolution='50m',color='black')
ax1.add_feature(bathym, facecolor='none', edgecolor='black', linestyle='dashed', linewidth=1)
# Weddell
...
cbars =fig.colorbar(cs, extend='max')
cv=ax2.quiver(xs[::30,::30],ys[::30,::30],np.nanmean(u_bot[:,::30,::30],axis=0),np.nanmean(v_bot[:,::30,::30],axis=0), color='gray', transform=src)
ax2.gridlines()
ax2.coastlines(resolution='50m',color='black')
ax2.add_feature(bathym, facecolor='none', edgecolor='black', linestyle='dashed', linewidth=1)
# Weddell
...
plt.savefig('/users/boeiradi/COLD_project/postprocessing/figs/WAOM2extend_currents+transects_RF-RIS.png', dpi=300)
plt.close()
...
plt.savefig('/users/boeiradi/COLD_project/postprocessing/figs/WAOM2extend_currents+transects_RFIS.png', dpi=300)
plt.close()
...
plt.savefig('/users/boeiradi/COLD_project/postprocessing/figs/WAOM2extend_currents+transects_RIS.png', dpi=300)
plt.close()
...
cs=plt.pcolormesh(lon_rho,lat_rho,np.nanmean(vel_bot, axis=0), transform=ccrs.PlateCarree(), cmap=plt.cm.GnBu, vmin=0, vmax=.5)
cbars =fig.colorbar(cs, extend='max')
cv=ax1.quiver(xs[::30,::30],ys[::30,::30],np.nanmean(u_bot[:,::30,::30],axis=0),np.nanmean(v_bot[:,::30,::30],axis=0), color='gray', transform=src)
ax1.gridlines()
ax1.coastlines(resolution='50m',color='black')
ax1.add_feature(bathym, facecolor='none', edgecolor='black', linestyle='dashed', linewidth=1)
# Weddell
...
plt.savefig('/users/boeiradi/COLD_project/postprocessing/figs/WAOM2extend_bottom_sfc_velTS+transects_RFIS.png', dpi=300)
plt.close()
...
plt.savefig('/users/boeiradi/COLD_project/postprocessing/figs/WAOM2extend_bottom_velTS+transects_RIS.png', dpi=300)
plt.close()
...
plt.savefig('/users/boeiradi/COLD_project/postprocessing/figs/WAOM2extend_bottom_velTS+transects_WAnt.png', dpi=300)
plt.close()
...
plt.savefig('/users/boeiradi/COLD_project/postprocessing/figs/WAOM2extend_bottom_velTS+transects_EAnt.png', dpi=300)
plt.close()
...
plt.savefig('/users/boeiradi/COLD_project/postprocessing/figs/WAOM2extend_NSIDC_September_sea-ice+transects_RF-RIS.png', dpi=300)

plt.show()

# Weddell Sea
# 1 - slice section for salt/temp/z_rho
section_salt_wedW = np.empty((12,31,600)) # allocating
section_temp_wedW = np.empty((12,31,600))
section_rho_wedW = np.empty((12,31,600))
section_z_wedW = np.empty((12,31,600))
section_z_wedW_mask = np.empty((12,31,600))
...
section_lat_wedW = np.ones((31,600))                                                
...
# Weddell Sea
# 1 - slice section for salt/temp/z_rho
section_salt_wedE = np.empty((12,31,600)) # allocating
section_temp_wedE = np.empty((12,31,600))
section_rho_wedE = np.empty((12,31,600))
section_z_wedE = np.empty((12,31,600))
section_z_wedE_mask = np.empty((12,31,600))
...
# 2.1 - mask land values for lon/lat needs loop for repeat through vert layers
section_lat_wedE = np.ones((31,600))                                                
for ii in np.arange(0,31):
...
# Ross Sea
# 1 - slice section for salt/temp/z_rho
section_salt_rosE = np.empty((12,31,325)) # allocating
section_temp_rosE = np.empty((12,31,325))
section_rho_rosE = np.empty((12,31,325))
section_z_rosE = np.empty((12,31,325))
section_z_rosE_mask = np.empty((12,31,325))
...
# 2.1 - mask land values for lon/lat needs loop for repeat through vert layers
section_lat_rosE = np.ones((31,325))                                                
for ii in np.arange(0,31):
...
# Ross Sea
# 1 - slice section for salt/temp/z_rho
section_salt_rosW = np.empty((12,31,696)) # allocating
section_temp_rosW = np.empty((12,31,696))
section_rho_rosW = np.empty((12,31,696))
section_z_rosW = np.empty((12,31,696))
section_z_rosW_mask = np.empty((12,31,696))
...
# 2.1 - mask land values for lon/lat needs loop for repeat through vert layers
section_lat_rosW = np.ones((31,696))                                                
for ii in np.arange(0,31):
...
name_fig="waom2extend_wedW_section_seas_yr20.png"
...
name_fig="waom2extend_wedE_section_seas_yr20.png"
plt.savefig(fig_path + name_fig)
...
name_fig="waom2extend_rosW_section_seas_yr20.png"
plt.savefig(fig_path + name_fig)
...
name_fig="waom2extend_rosE_section_seas_yr20.png"
plt.savefig(fig_path + name_fig)
...
name_fig="waom2extend_RF-RISsections_annual_yr20.png"
plt.savefig(fig_path + name_fig)
