...
#ds = xr.open_dataset("/g/data3/hh5/tmp/access-om/fbd581/ROMS/OUTPUT/waom10_shflim_S_ORAS5em_0.25Q/output_01-20yr/ocean_avg_0010.nc")
ds = xr.open_dataset("/scratch/gh9/fbd581/ROMS/waom10_shflim_S_ORAS5em_0.25Q/output_01-20yr/ocean_avg_0010.nc")
...
print(z_rho.shape)
print(ds.temp.shape)
print(lon_rho[280:450,200].shape)
print(np.ones((170,31)).shape)

# plot some sections: Salt
# Weddell Sea
section_rho_wed = np.nanmean(np.squeeze(sigma_t[:,:,280:450,170]), axis=0)
section_salt_wed = np.nanmean(ds.salt.isel(xi_rho=170, eta_rho=slice(280,450)), axis=0)
section_temp_wed = np.nanmean(ds.temp.isel(xi_rho=170, eta_rho=slice(280,450)), axis=0)
section_lat_wed_tmp = ds.lat_rho.isel(xi_rho=170, eta_rho=slice(280,450))
section_z_wed = np.nanmean(ds.z_rho.isel(xi_rho=170, eta_rho=slice(280,450)), axis=2)
...
# Ross Sea
section_rho_ros = np.nanmean(np.squeeze(sigma_t[:,:,20:250,310]), axis=0)
section_salt_ros = np.nanmean(ds.salt.isel(xi_rho=310, eta_rho=slice(20,250)), axis=0)
section_temp_ros = np.nanmean(ds.temp.isel(xi_rho=310, eta_rho=slice(20,250)), axis=0)
section_lat_ros_tmp = ds.lat_rho.isel(xi_rho=310, eta_rho=slice(20,250))
section_z_ros = np.nanmean(ds.z_rho.isel(xi_rho=310, eta_rho=slice(20,250)), axis=2)
...
# Prydz Bay
section_rho_pry = np.nanmean(np.squeeze(sigma_t[:,:,338,420:600]), axis=0)
section_salt_pry = np.nanmean(ds.salt.isel(xi_rho=slice(420,600), eta_rho=338), axis=0)
section_temp_pry = np.nanmean(ds.temp.isel(xi_rho=slice(420,600), eta_rho=338), axis=0)
section_lon_pry_tmp = ds.lon_rho.isel(xi_rho=slice(420,600), eta_rho=338)
section_z_pry = np.nanmean(ds.z_rho.isel(xi_rho=slice(420,600), eta_rho=338), axis=2)
...
# plot some sections: Salt
# Maud Land1
section_rho_neu = np.nanmean(np.squeeze(sigma_t[:,:,420:520,280]), axis=0)
section_salt_neu = np.nanmean(ds.salt.isel(xi_rho=280, eta_rho=slice(420,520)), axis=0)
section_temp_neu = np.nanmean(ds.temp.isel(xi_rho=280, eta_rho=slice(420,520)), axis=0)
section_lat_neu_tmp = ds.lat_rho.isel(xi_rho=280, eta_rho=slice(420,520))
section_z_neu = np.nanmean(ds.z_rho.isel(xi_rho=280, eta_rho=slice(420,520)), axis=2)
...
section_rho_mai = np.nanmean(np.squeeze(sigma_t[:,:,420:520,320]), axis=0)
section_salt_mai = np.nanmean(ds.salt.isel(xi_rho=320, eta_rho=slice(420,520)), axis=0)
section_temp_mai = np.nanmean(ds.temp.isel(xi_rho=320, eta_rho=slice(420,520)), axis=0)
section_lat_mai_tmp = ds.lat_rho.isel(xi_rho=320, eta_rho=slice(420,520))
section_z_mai = np.nanmean(ds.z_rho.isel(xi_rho=320, eta_rho=slice(420,520)), axis=2)
...
# Merz Glacier
section_rho_mer = np.nanmean(np.squeeze(sigma_t[:,:,10:80,435]), axis=0)
section_salt_mer = np.nanmean(ds.salt.isel(xi_rho=435, eta_rho=slice(10,80)), axis=0)
section_temp_mer = np.nanmean(ds.temp.isel(xi_rho=435, eta_rho=slice(10,80)), axis=0)
section_lat_mer_tmp = ds.lat_rho.isel(xi_rho=435, eta_rho=slice(10,80))
section_z_mer = np.nanmean(ds.z_rho.isel(xi_rho=435, eta_rho=slice(10,80)), axis=2)
...
# Darnley Polynya
section_rho_dar = np.nanmean(np.squeeze(sigma_t[:,:,350,460:615]), axis=0)
section_salt_dar = np.nanmean(ds.salt.isel(xi_rho=slice(460,615), eta_rho=350), axis=0)
section_temp_dar = np.nanmean(ds.temp.isel(xi_rho=slice(460,615), eta_rho=350), axis=0)
section_lat_dar_tmp = ds.lat_rho.isel(xi_rho=slice(460,615), eta_rho=350)
section_z_dar = np.nanmean(ds.z_rho.isel(xi_rho=slice(460,615), eta_rho=350), axis=2)
...
# Amundsen Sea
section_rho_amu = np.nanmean(np.squeeze(sigma_t[:,:,50:200,195]), axis=0)
section_salt_amu = np.nanmean(ds.salt.isel(eta_rho=195, xi_rho=slice(50,200)), axis=0)
section_temp_amu = np.nanmean(ds.temp.isel(eta_rho=195, xi_rho=slice(50,200)), axis=0)
section_lat_amu_tmp = ds.lat_rho.isel(eta_rho=195, xi_rho=slice(50,200))
section_z_amu = np.nanmean(ds.z_rho.isel(eta_rho=195, xi_rho=slice(50,200)), axis=2)
...
ds[variables].isel(ocean_time=slice(0, 12), #wed
                  xi_rho=170, eta_rho=slice(280,450)).to_netcdf(ncdftmp_path + 'WAOM_shflim_S_ORAS5_0.25Q_sec_wed_ann_yr10.nc', mode='w')

ds[variables].isel(ocean_time=slice(0, 12), # ros
                  xi_rho=310, eta_rho=slice(20,250)).to_netcdf(ncdftmp_path + 'WAOM_shflim_S_ORAS5_0.25Q_sec_ros_ann_yr10.nc', mode='w')

ds[variables].isel(ocean_time=slice(0, 12), # pry
                  xi_rho=slice(420,600), eta_rho=338).to_netcdf(ncdftmp_path + 'WAOM_shflim_S_ORAS5_0.25Q_sec_pry_ann_yr10.nc', mode='w')

ds[variables].isel(ocean_time=slice(0, 12), # neu
                  xi_rho=280, eta_rho=slice(420,520)).to_netcdf(ncdftmp_path + 'WAOM_shflim_S_ORAS5_0.25Q_sec_neu_ann_yr10.nc', mode='w')

ds[variables].isel(ocean_time=slice(0, 12), # mai
                  xi_rho=320, eta_rho=slice(420,520)).to_netcdf(ncdftmp_path + 'WAOM_shflim_S_ORAS5_0.25Q_sec_mai_ann_yr10.nc', mode='w')

ds[variables].isel(ocean_time=slice(0, 12), # dar
                  eta_rho=350, xi_rho=slice(460,615)).to_netcdf(ncdftmp_path + 'WAOM_shflim_S_ORAS5_0.25Q_sec_dar_ann_yr10.nc', mode='w')

ds[variables].isel(ocean_time=slice(0, 12), # mer
                  xi_rho=435, eta_rho=slice(10,80)).to_netcdf(ncdftmp_path + 'WAOM_shflim_S_ORAS5_0.25Q_sec_mer_ann_yr10.nc', mode='w')

ds[variables].isel(ocean_time=slice(0, 12), # amu
                  eta_rho=195, xi_rho=slice(50,200)).to_netcdf(ncdftmp_path + 'WAOM_shflim_S_ORAS5_0.25Q_sec_amu_ann_yr10.nc', mode='w')

print('saved')

# load vars for TS plot
wed=ncdftmp_path + 'WAOM_shflim_S_ORAS5_0.25Q_sec_wed_ann_yr10.nc'
...
ros=ncdftmp_path + 'WAOM_shflim_S_ORAS5_0.25Q_sec_ros_ann_yr10.nc'
...
pry=ncdftmp_path + 'WAOM_shflim_S_ORAS5_0.25Q_sec_pry_ann_yr10.nc'
...
neu=ncdftmp_path + 'WAOM_shflim_S_ORAS5_0.25Q_sec_neu_ann_yr10.nc'
...
mai=ncdftmp_path + 'WAOM_shflim_S_ORAS5_0.25Q_sec_mai_ann_yr10.nc'
...
mer=ncdftmp_path + 'WAOM_shflim_S_ORAS5_0.25Q_sec_mer_ann_yr10.nc'
...
dar=ncdftmp_path + 'WAOM_shflim_S_ORAS5_0.25Q_sec_dar_ann_yr10.nc'
...
amu=ncdftmp_path + 'WAOM_shflim_S_ORAS5_0.25Q_sec_amu_ann_yr10.nc'
...
name_fig="waom10_shflim_S_ORAS5em_0.25Q_AllDSWsections_TSdiag_depth_yr10.png"
...
name_fig="waom10_shflim_S_ORAS5_0.25Q_DSW+sections_TSdiag_depth_yr10.png"
plt.savefig(fig_path + name_fig, dpi=300)
