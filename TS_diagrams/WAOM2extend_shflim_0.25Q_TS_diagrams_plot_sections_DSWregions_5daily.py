...
# load section vars:
wed = '/users/boeiradi/COLD_project/postprocessing/ncdf_tmp/WAOM2extend_shflim_S_sec_wed_5days_annual.nc'
print(wed)
...
lat_pry_tmp = dx.lat_rho.isel(eta_rho=1840, xi_rho=slice(2100,3000))
z_pry_mask = ma.array(xr.DataArray.from_iris(z_rho_pry),mask=np.isnan(xr.DataArray.from_iris(z_rho_pry)))
lat_pry = np.ones((31,900))
for ii in np.arange(0,31):
    lat_pry[ii,:] = lat_pry_tmp
lat_pry_mask =  ma.array(lat_pry,mask=np.isnan(lat_pry))

neu = '/users/boeiradi/COLD_project/postprocessing/ncdf_tmp/WAOM2extend_shflim_S_sec_neu_5days_annual.nc'
#sec = iris.load('/users/boeiradi/COLD_project/postprocessing/ncdf_tmp/WAOM2extend_shflim_S_ORAS5em_0.25Q_sec_neu_5days_annual.nc')
print(neu)
...
lat_neu_tmp = dx.lat_rho.isel(xi_rho=1400, eta_rho=slice(2250,2750))
z_neu_mask = ma.array(xr.DataArray.from_iris(z_rho_neu),mask=np.isnan(xr.DataArray.from_iris(z_rho_neu)))
lat_neu = np.ones((31,500))
for ii in np.arange(0,31):
    lat_neu[ii,:] = lat_neu_tmp
lat_neu_mask =  ma.array(lat_neu,mask=np.isnan(lat_neu))

mai = '/users/boeiradi/COLD_project/postprocessing/ncdf_tmp/WAOM2extend_shflim_S_sec_mai_5days_annual.nc'
#sec = iris.load('/users/boeiradi/COLD_project/postprocessing/ncdf_tmp/WAOM2extend_shflim_S_ORAS5em_0.25Q_sec_mai_5days_annual.nc')
print(mai)
...
lat_mer_tmp = dx.lat_rho.isel(xi_rho=2176, eta_rho=slice(200,550))
z_mer_mask = ma.array(xr.DataArray.from_iris(z_rho_mer),mask=np.isnan(xr.DataArray.from_iris(z_rho_mer)))
lat_mer = np.ones((31,350))
for ii in np.arange(0,31):
    lat_mer[ii,:] = lat_mer_tmp
lat_mer_mask =  ma.array(lat_mer,mask=np.isnan(lat_mer))

amu = '/users/boeiradi/COLD_project/postprocessing/ncdf_tmp/WAOM2extend_shflim_S_sec_amu_5days_annual.nc'
#sec = iris.load('/users/boeiradi/COLD_project/postprocessing/ncdf_tmp/WAOM2extend_shflim_S_ORAS5em_0.25Q_sec_amu_5days_annual.nc')
print(amu)
...
name_fig="waom2extend_shflim_S_0.25Q_DSWsections_TSdiag_depth_yr20.png"
...
name_fig="waom2extend_shflim_S_0.25Q_AllDSWsections_TSdiag_depth_yr20.png"
plt.savefig(fig_path + name_fig, dpi=300)
...
name_fig="waom2extend_shflim_S_0.25Q_DSW+sections_TSdiag_depth_yr20.png"
plt.savefig(fig_path + name_fig, dpi=300)
