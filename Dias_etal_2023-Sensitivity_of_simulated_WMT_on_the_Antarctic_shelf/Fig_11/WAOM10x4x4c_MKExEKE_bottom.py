...
def read_roms_ts_10km(exp_path):
    for yr  in ['20']:
        ...
        # 1. load bottom vel
        mmax = 12
        ubot = ds.variables["u"].isel(s_rho=0)
        vbot = ds.variables["v"].isel(s_rho=0)
        ...
        wf = lambda r: 1/r
        uboti = np.empty(ds.salt.isel(s_rho=0).shape)
        vboti = np.empty(ds.salt.isel(s_rho=0).shape)
        for mm in np.arange(0,mmax):
            uboti[mm,:,:] = pyresample.kd_tree.resample_custom(w10u_def,ubot[mm,:,:].values,w10_def,\
                                             radius_of_influence=5000,neighbours=1,weight_funcs=wf)
            vboti[mm,:,:] = pyresample.kd_tree.resample_custom(w10v_def,vbot[mm,:,:].values,w10_def,\
                                             radius_of_influence=5000,neighbours=1,weight_funcs=wf)

        ubot_ann_tmp = np.nanmean(uboti, axis=0)
        vbot_ann_tmp = np.nanmean(vboti, axis=0)
        V_ann_tmp = np.sqrt(ubot_ann_tmp*ubot_ann_tmp + vbot_ann_tmp*vbot_ann_tmp)
        MKE_tmp = np.divide(V_ann_tmp*V_ann_tmp,2)
        print("Size MKE_tmp", MKE_tmp.shape)
        
        ubot_anom_tmp = uboti - ubot_ann_tmp
        vbot_anom_tmp = vboti - vbot_ann_tmp
        V_anom_tmp = np.sqrt(ubot_anom_tmp*ubot_anom_tmp + vbot_anom_tmp*vbot_anom_tmp)
...
def read_roms_ts_4km(exp_path,yr_file):
    for yr in yr_file:
        ...
        vboti = np.empty(ds.salt.isel(s_rho=0).shape)
        for mm in np.arange(0,mmax):
            uboti[mm,:,:] = pyresample.kd_tree.resample_custom(w4u_def,ubot[mm,:,:].values,w4_def,\
                                             radius_of_influence=5000,neighbours=1,weight_funcs=wf)
            vboti[mm,:,:] = pyresample.kd_tree.resample_custom(w4v_def,vbot[mm,:,:].values,w4_def,\
                                             radius_of_influence=5000,neighbours=1,weight_funcs=wf)
...
def read_roms_ts_2km(exp_path):
    for yr in ['09','10']:
        ...
        vboti = np.empty(ds.salt.isel(s_rho=0).shape)
        for mm in np.arange(0,mmax):
            uboti[mm,:,:] = pyresample.kd_tree.resample_custom(w2u_def,ubot[mm,:,:].values,w2_def,\
                                             radius_of_influence=5000,neighbours=1,weight_funcs=wf)
            vboti[mm,:,:] = pyresample.kd_tree.resample_custom(w2v_def,vbot[mm,:,:].values,w2_def,\
                                             radius_of_influence=5000,neighbours=1,weight_funcs=wf)
...
plt.title('MKE \n WAOM10')
ax1.gridlines(zorder=4,draw_labels=True, dms=False, x_inline=False, y_inline=False) # draw_labels=True,linewidth=
...
ax2.gridlines(zorder=4,draw_labels=True, dms=False, x_inline=False, y_inline=False)
#lonlat_labels(ax2)
...
#cbart =fig.colorbar(ct)# , extend='both')
plt.title('WAOM4')
ax3.gridlines(zorder=4,draw_labels=True, dms=False, x_inline=False, y_inline=False) # draw_labels=True,linewidth=
...
ax4.gridlines(zorder=4,draw_labels=True, dms=False, x_inline=False, y_inline=False)
#lonlat_labels(ax4)
...
#cbart =fig.colorbar(ct)# , extend='both')
plt.title('WAOM2')
ax5.gridlines(zorder=4,draw_labels=True, dms=False, x_inline=False, y_inline=False) # draw_labels=True,linewidth=
...
ax6.gridlines(zorder=4,draw_labels=True, dms=False, x_inline=False, y_inline=False)
#lonlat_labels(ax6)
...
name_fig="waom10x4x2extend_shflim_S_0.25Q_MKExEKE_bottom_maps.png"
...
plt.contour(lon_rho_2km[xlimit,ylimit], lat_rho_2km[xlimit,ylimit],zice_2km[xlimit,ylimit],levels=[-.1],linestyles='dashed', transform=ccrs.PlateCarree(), cmap=plt.cm.binary, linewidths=1)
...
name_fig="waom10x4x2extend_shflim_S_0.25Q_MKE-EKE_bottom_RFIS_anomalies.png"
plt.savefig(fig_path + name_fig, dpi=300)
plt.close()
...
ax1 = fig.add_subplot(321, projection=proj)
ct1=plt.pcolormesh(lon_rho_10km,lat_rho_10km,np.squeeze(MKE_10km[:,:])*1e2, transform=ccrs.PlateCarree(), cmap=plt.cm.plasma, vmin=MEmin, vmax=MEmax)
...
cbar_ax0 = fig.add_axes([0.05, 0.685, 0.01, 0.16])
fig.colorbar(ct1, cax=cbar_ax0, orientation='vertical')
cbar_ax0.set_ylabel('MKE (10$^{-2}$ m$^{2}$ s$^{-2}$)', fontsize=15, labelpad=-90)
cbar_ax0.tick_params(labelsize=15)

ax2 = fig.add_subplot(322, projection=proj)
cs1=plt.pcolormesh(lon_rho_10km,lat_rho_10km,np.squeeze(EKE_10km[:,:])*1e2, transform=ccrs.PlateCarree(), cmap=plt.cm.plasma, vmin=EKmin, vmax=EKmax)
...
cbar_ax2.set_ylabel('EKE (10$^{-2}$ m$^{2}$ s$^{-2}$)', fontsize=15)#, labelpad=-90)
...
ax3 = fig.add_subplot(323, projection=proj)
ct2=plt.pcolormesh(lon_rho_10km,lat_rho_10km,(MKE_4km_interp - np.squeeze(MKE_10km[:,:]))*1e2, transform=ccrs.PlateCarree(), cmap=plt.cm.coolwarm, vmin=MEmina, vmax=MEmaxa)
plt.title('C) WAOM4 - WOM10', fontsize=16)
gl = ax3.gridlines(zorder=4,draw_labels=True, dms=False, x_inline=False, y_inline=False) # draw_labels=True,linewidth=
gl.rotate_labels = False
gl.ylabels_right = False
gl.xlabels_top = False
gl.xlines = True
...
fig.colorbar(ct2, cax=cbar_ax1, orientation='vertical')
cbar_ax1.set_ylabel('MKE anomaly (10$^{-2}$ m$^{2}$ s$^{-2}$)', fontsize=15, labelpad=-90)
cbar_ax1.tick_params(labelsize=15)

ax4 = fig.add_subplot(324, projection=proj)
cs2=plt.pcolormesh(lon_rho_10km,lat_rho_10km,(EKE_4km_interp - np.squeeze(EKE_10km[:,:]))*1e3, transform=ccrs.PlateCarree(), cmap=plt.cm.coolwarm, vmin=EKmina, vmax=EKmaxa)
...
ax5 = fig.add_subplot(325, projection=proj)
ct2=plt.pcolormesh(lon_rho_10km,lat_rho_10km,(MKE_4kmB_interp - np.squeeze(MKE_10km[:,:]))*1e2, transform=ccrs.PlateCarree(), cmap=plt.cm.coolwarm, vmin=MEmina, vmax=MEmaxa)
plt.title('E) WAOM4-COARSE - WAOM10', fontsize=16)
...
ax6 = fig.add_subplot(326, projection=proj)
cs2=plt.pcolormesh(lon_rho_10km,lat_rho_10km,(EKE_4kmB_interp - np.squeeze(EKE_10km[:,:]))*1e3, transform=ccrs.PlateCarree(), cmap=plt.cm.coolwarm, vmin=EKmina, vmax=EKmaxa)
...
fig.colorbar(cs2, cax=cbar_ax3, orientation='vertical')
cbar_ax3.set_ylabel('EKE anomaly (10$^{-3}$ m$^{2}$ s$^{-2}$)', fontsize=15)#, labelpad=-35)
cbar_ax3.tick_params(labelsize=15)


name_fig="waom10x4x4cextend_shflim_S_0.25Q_MKE-EKE_bottom_EAnt_anomalies.png"
...
cbar_ax3 = fig.add_axes([0.88, 0.53, 0.01, 0.35])
fig.colorbar(ct1, cax=cbar_ax3, orientation='vertical')
cbar_ax3.set_xlabel('MKE (m$^{2}$ s$^{-2}$)', fontsize=15, labelpad=-90)
cbar_ax3.tick_params(labelsize=15)
...
cbar_ax4 = fig.add_axes([0.88, 0.12, 0.01, 0.35])
fig.colorbar(cs1, cax=cbar_ax4, orientation='vertical')
cbar_ax4.set_xlabel('EKE (m$^{2}$ s$^{-2}$)', fontsize=15, labelpad=-90)
...
name_fig="waom10x4x2extend_shflim_S_0.25Q_MKE-EKE_bottom_WAnt_anomalies.png"
plt.savefig(fig_path + name_fig, dpi=300)
plt.close()
