...
def read_roms_ts(exp_path):
    ...
    dz = np.empty((10,560,630,31))
    dz_inv = np.empty((10,560,630,31))
    ...
    ohc_dz = np.empty((10,531,560,630))
    salt_dz = np.empty((10,531,560,630))
    ...
    ohc_area = np.empty((10,560,630))
    salt_area = np.empty((10,560,630))
    ohc_area_shelf = np.empty((10,560,630))
    salt_area_shelf = np.empty((10,560,630))
    ohc_area_open = np.empty((10,560,630))
    salt_area_open = np.empty((10,560,630))
...
path_ORAS5 = '/scratch/gh9/fbd581/ROMS/waom10extend_shflim_S_0.25Q_ORAS5em/output_01-10yr/'
path_ECCO2 = '/scratch/gh9/fbd581/ROMS/waom10extend_shflim_S_0.25Q/output_01-20yr/'

O_ohc_hintn, O_salt_hintn, O_shflx_hintn, O_sfwflx_hintn, O_MKE_havg, O_SST_havg, O_SSS_havg, O_ohc_hintn_shelf, O_ohc_hintn_open, O_salt_hintn_shelf, O_salt_hintn_open = read_roms_ts(path_ORAS5)
E_ohc_hintn, E_salt_hintn, E_shflx_hintn, E_sfwflx_hintn, E_MKE_havg, E_SST_havg, E_SSS_havg, E_ohc_hintn_shelf, E_ohc_hintn_open, E_salt_hintn_shelf, E_salt_hintn_open = read_roms_ts(path_ECCO2)

fig_path='/g/data3/hh5/tmp/access-om/fbd581/ROMS/postprocessing/figs/Grid_avg/'

fig = plt.figure(figsize=(12,8))
ax1 = fig.add_subplot(311)
plt.plot(np.arange(0,10)+1,O_ohc_hintn,'b',label='ORAS5 10km')
plt.plot(np.arange(0,10)+1,E_ohc_hintn,'--r',label='ECCO2 10km')
plt.text(15.,-6.e9,'$\overline{Q}_{ORAS5}$ = ' +  str(np.around(O_shflx_hintn, decimals=0)) +' W m$^{-2}$')
plt.text(15.,-6.5e9,'$\overline{Q}_{ECCO2}$ = ' +  str(np.around(E_shflx_hintn, decimals=0)) +' W m$^{-2}$')
...
plt.plot(np.arange(0,10)+1,O_salt_hintn,'b',label='ORAS5 10km')
plt.plot(np.arange(0,10)+1,E_salt_hintn,'--r',label='ECCO2 10km')
plt.text(15.,-60130,'$\overline{FWF}_{ORAS5}$ = ' +  str(np.around(O_sfwflx_hintn, decimals=0)) +' m s$^{-1}$')
plt.text(15.,-60160,'$\overline{FWF}_{ECCO2}$ = ' +  str(np.around(E_sfwflx_hintn, decimals=0)) +' m s$^{-1}$')
#l2 = plt.legend(bbox_to_anchor=(1.04,0.5), loc="center left", borderaxespad=0)
plt.ylabel('salt content')
ax3 = fig.add_subplot(313)
plt.plot(np.arange(0,10)+1,O_MKE_havg,'b',label='ORAS5 10km')
plt.plot(np.arange(0,10)+1,E_MKE_havg,'--r',label='ECCO2 10km')
#l3 = plt.legend(bbox_to_anchor=(1.04,0.5), loc="center left", borderaxespad=0)
plt.ylabel('MKE (m$^{2}$ s$^{-2}$)')
plt.xlabel('years')

name_fig="waom10extend_shflim_S_ORAS5emXECCO2_0.25Q_OHC+Salt+MKE_evolution.png"
plt.savefig(fig_path + name_fig, dpi=300)
plt.close()

fig = plt.figure(figsize=(12,8))
ax1 = fig.add_subplot(211)
plt.plot(np.arange(0,10)+1,O_SST_havg,'b',label='ORAS5 10km')
plt.plot(np.arange(0,10)+1,E_SST_havg,'--r',label='ECCO2 10km')
#l3 = plt.legend(bbox_to_anchor=(1.04,0.5), loc="center left", borderaxespad=0)
l3 = plt.legend(loc="upper left", borderaxespad=0)
plt.ylabel('SST')
ax2 = fig.add_subplot(212)
plt.plot(np.arange(0,10)+1,O_SSS_havg,'b',label='ORAS5 10km')
plt.plot(np.arange(0,10)+1,E_SSS_havg,'--r',label='ECCO2 10km')
#l3 = plt.legend(bbox_to_anchor=(1.04,0.5), loc="center left", borderaxespad=0)
plt.ylabel('SSS')

name_fig="waom10extend_shflim_S_ORAS5emXECCO2_0.25Q_SST+SSS_evolution.png"
plt.savefig(fig_path + name_fig, dpi=300)
plt.close()

#fig = plt.figure(figsize=(14,10))
#ax1 = fig.add_subplot(211)
#plt.plot(np.arange(0,10)+1,ohc_hintn,'b',label='10km total')
#plt.plot(np.arange(0,10)+1,ohc_hintn_shelf,':b',label='10km shelf')
#plt.plot(np.arange(0,10)+1,ohc_hintn_open,'-*b',label='10km open-ocean')
#l1 = plt.legend(bbox_to_anchor=(0.7,0.72), loc="center left", borderaxespad=0)
#plt.ylabel('heat content (J)')
#ax3 = fig.add_subplot(212)
#plt.plot(np.arange(0,10)+1,salt_hintn,'r',label='10km total')
#plt.plot(np.arange(0,10)+1,salt_hintn_shelf,':r',label='10km shelf')
#plt.plot(np.arange(0,10)+1,salt_hintn_open,'-*r',label='10km open-ocean')
#l3 = plt.legend(bbox_to_anchor=(0.7,0.72), loc="center left", borderaxespad=0)
#
#name_fig="waom10extend_shflim_S_ORAS5em_0.25Q_OHC+Salt_subregions_evolution.png"
#plt.savefig(fig_path + name_fig, dpi=300)
#plt.close()
