...
ax1 = fig.add_subplot(411)
plt.plot(np.arange(0,10)+1,(AOHC_E4)*1e-9,'-b',label='WAOM4')
plt.plot(np.arange(0,10)+1,(AOHC_E4t)*1e-9,'-g',label='WAOM4-NOTIDE')
plt.plot(np.arange(0,10)+1,np.zeros(E4_ohc_hintn_shelf.shape),'-k')
plt.text(1.1,0.05,'(A)',fontsize=16,weight="bold")
l1 = plt.legend(loc='upper left', borderaxespad=1)
plt.ylabel('OHC (GJ)',fontsize=14)
plt.grid('on')
plt.xticks(np.arange(0, 11, step=1)),plt.xlim([1,10])
ax2 = fig.add_subplot(412)
plt.plot(np.arange(0,10)+1,AOSC_E4,'-b',label='WAOM4')
...
plt.plot(np.arange(0,10)+1,(AMKE_E4)*1e3,'-b',label='WAOM4')
