...
dfw = xr.open_dataset("/scratch/project_2000339/boeiradi/waom10_frc/waom10_swflux.nc")
dshf = xr.open_dataset("/scratch/project_2000339/boeiradi/waom10_frc/waom10_shflux.nc")

## --------------------- load EN4 data:
EN_temp = np.empty((0,42,173,360))
EN_salt = np.empty((0,42,173,360))
EN_temp_mm = np.empty((12,42,173,360))
EN_salt_mm = np.empty((12,42,173,360))

for yr in ['2005','2006','2007','2008','2009','2010','2011','2012','2013','2014','2015']:
    count = 0
    for mm in ['01','02','03','04','05','06','07','08','09','10','11','12']:
        de_tmp = xr.open_dataset("/scratch/project_2000339/boeiradi/EN/EN.4.2.1.f.analysis.l09." + yr + mm + ".nc")
        EN_temp_mm[count,:] = de_tmp.temperature - 273.15
        EN_salt_mm[count,:] = de_tmp.salinity
        count = count +1
	#print('EN monthly size:', EN_temp_mm.shape)
    print('EN monthly size:', EN_temp_mm.shape)   
    EN_temp = np.concatenate((EN_temp, EN_temp_mm), axis=0)
    EN_salt = np.concatenate((EN_salt, EN_salt_mm), axis=0)
    print('EN annual size:', EN_temp.shape)
...
ind_bot = np.empty([132,173,360])
EN_temp_bot = np.empty([132,173,360])
EN_salt_bot = np.empty([132,173,360])

for tt in np.arange(0,132):
...
def lonlat_labels(ax):
    ...
    ax.text(120,-75,'70$^{\circ}$S',transform=ccrs.PlateCarree(),color='gray')
    # longitude labels
    ax.text(0,-64,'0$^{\circ}$',transform=ccrs.PlateCarree(),color='gray')
...
#cc =plt.imshow(ice_xgrid, ice_ygrid,np.squeeze(ice_mm[9,:,:]), transform=ccrs.Stereographic(**kw), vmin=0, vmax=1) #
...
#ct=plt.contourf(lon_rho,lat_rho,temp_sfc, levels=levelsT, transform=ccrs.PlateCarree(), cmap=plt.cm.coolwarm, vmin=tmin, vmax=tmax)
...
ci=plt.contour(ice_xgrid, ice_ygrid,np.squeeze(ice_mm[8,:,:]), levels=[.75],colors='dimgray', transform=ccrs.Stereographic(**kw)) #
ci=plt.contour(ice_xgrid, ice_ygrid,np.squeeze(ice_mm[2,:,:]), levels=[.75],colors='white', transform=ccrs.Stereographic(**kw)) #
#cs=plt.contourf(lon_rho,lat_rho,salt_sfc, levels=levelsS, transform=ccrs.PlateCarree(), cmap=plt.cm.coolwarm, vmin=smin, vmax=smax)
...
#ct2=plt.contourf(EN_lon[:,:22],EN_lat[:,:22],np.transpose(EN_temp_sfc[:22,:]),levels=levelsT, transform=ccrs.PlateCarree(), cmap=plt.cm.coolwarm, vmin=tmin, vmax=tmax)
...
#cs2=plt.contourf(EN_lon[:,:22],EN_lat[:,:22],np.transpose(EN_salt_sfc[:22,:]),levels=levelsS,transform=ccrs.PlateCarree(), cmap=plt.cm.coolwarm, vmin=smin, vmax=smax)
...
plt.savefig('figs/WAOM10/waom10xEN4_surface_TS_annual.png')
plt.close(fig)
...
Hsbl = ds.Hsbl.isel(ocean_time=9) #mean(axis=0)
print('temp,Hsbl size=', temp_bot.shape, Hsbl.shape)
...
plt.savefig('figs/WAOM10/waom10xEN4_bottom_TS_annual.png')
plt.close(fig)
