...
# saving all variables including temp/salt
            ds[variables].isel(ocean_time=slice(0, 8),xi_rho=xi_pt[ind_ij], eta_rho=slice(eta_sec_ini[ind_ij],eta_sec_end[ind_ij])).to_netcdf('/scratch/project_2000339/boeiradi/postprocessing/ncdf_tmp/WAOM4_sec_' + [variables] + sec_name + '_5days_mm' + mm + '.nc', mode='w', format='NETCDF3_64BIT')
            ...
            for vv in np.arange(0,3):
                ...
                if vv <= 1:
                    ...
                    var_str = 'zeta'
                ds[var].isel(ocean_time=slice(0, 8),xi_rho=slice(xi_sec_ini[ind_ij],xi_sec_end[ind_ij]), eta_rho=eta_pt[ind_ij]).to_netcdf('/scratch/project_2000339/boeiradi/postprocessing/ncdf_tmp/WAOM4_sec_' + var_str + '_' + sec_name + '_5days_mm' + mm + '.nc', mode='w', format='NETCDF4')
