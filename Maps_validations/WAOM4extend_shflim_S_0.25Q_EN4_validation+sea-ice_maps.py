...
# load ROMS avg output
ds = xr.open_dataset("/scratch/project_2000789/boeiradi/waom4extend_shflim_S_0.25Q/output_01-10yr/ocean_avg_0010.nc")
...
ct=plt.pcolormesh(lon_rho,lat_rho,temp_sfc, transform=ccrs.PlateCarree(), cmap=plt.cm.coolwarm, vmin=tmin, vmax=tmax, shading='flat')
...
ci=plt.contour(ice_xgrid, ice_ygrid,np.squeeze(ice_mm[8,:,:]), levels=[.75],colors='gray', transform=ccrs.Stereographic(**kw)) #
ci=plt.contour(ice_xgrid, ice_ygrid,np.squeeze(ice_mm[2,:,:]), levels=[.75],colors='white', transform=ccrs.Stereographic(**kw)) #
cs=plt.pcolormesh(lon_rho,lat_rho,salt_sfc, transform=ccrs.PlateCarree(), cmap=plt.cm.coolwarm, vmin=smin, vmax=smax, shading='flat')
...
ct2=plt.pcolormesh(EN_lon[:,:22],EN_lat[:,:22],np.transpose(EN_temp_sfc[:22,:]), transform=ccrs.PlateCarree(), cmap=plt.cm.coolwarm, vmin=tmin, vmax=tmax, shading='flat')
...
cs2=plt.pcolormesh(EN_lon[:,:22],EN_lat[:,:22],np.transpose(EN_salt_sfc[:22,:]), transform=ccrs.PlateCarree(), cmap=plt.cm.coolwarm, vmin=smin, vmax=smax, shading='flat')
...
#plt.tight_layout()
name_fig = 'waom4extend_shflim_S_0.25QxEN4_surface_TS_annual_yr20.png'
...
ax1 = fig.add_subplot(221, projection=proj)
ct=plt.pcolormesh(lon_rho,lat_rho,temp_bot, transform=ccrs.PlateCarree(), cmap=plt.cm.coolwarm, vmin=tmin, vmax=tmax, shading='flat')
cbart =fig.colorbar(ct)# , extend='both')
...
ax2 = fig.add_subplot(222, projection=proj)
cs=plt.pcolormesh(lon_rho,lat_rho,salt_bot, transform=ccrs.PlateCarree(), cmap=plt.cm.coolwarm, vmin=smin, vmax=smax, shading='flat')
cbars =fig.colorbar(cs)# , extend='both')
...
ax3 = fig.add_subplot(223, projection=proj)
ct2=plt.pcolormesh(EN_lon[:,:22],EN_lat[:,:22],np.transpose(EN_temp_mbot[:22,:]), transform=ccrs.PlateCarree(), cmap=plt.cm.coolwarm, vmin=tmin, vmax=tmax, shading='flat')
cbart =fig.colorbar(ct2)# , extend='both')
...
ax4 = fig.add_subplot(224, projection=proj)
cs2=plt.pcolormesh(EN_lon[:,:22],EN_lat[:,:22],np.transpose(EN_salt_mbot[:22,:]), transform=ccrs.PlateCarree(), cmap=plt.cm.coolwarm, vmin=smin, vmax=smax, shading='flat')
cbars =fig.colorbar(cs2)# , extend='both')
...
#plt.tight_layout()
name_fig ='waom4extend_shflim_S_0.25QxEN4_bottom_TS_annual_yr20.png'
plt.savefig(fig_path + name_fig, dpi=300)
plt.close()
...
#    month=str(mm+1)
#
#    name_fig='waom4extend_shflim_S_0.25QxEN4_surface_TS_m=' + month + '_yr15.png'
...
#
#    
#    name_fig='waom4extend_shflim_S_0.25QxEN4_bottom_TS_m=' + month + '_yr15.png'
#    plt.savefig(fig_path + name_fig, dpi=300)
